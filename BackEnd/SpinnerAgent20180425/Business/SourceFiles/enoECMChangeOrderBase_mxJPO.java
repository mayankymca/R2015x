...
public class enoECMChangeOrderBase_mxJPO extends emxDomainObject_mxJPO {
	...
	/**
	 * @author
	 * Generates dynamic query for Review Route Template field
	 * @param context
	 * @param args
	 * @return String
	 * @throws Exception
	...
	private StringList getTypeAndPolicy(Context context, Map fieldValuesMap)throws Exception {
		...
		String result = MqlUtil.mqlCommand(context, "print connection $1 select $2 $3 $4 dump",
                                                   relId, "to.id", "to.type", "to.policy");
    ...
    public String getSearchQueryForTransferOwnership(Context context, String[] args) throws Exception
    {
		...
		if(!UIUtil.isNullOrEmpty(objectId)){
			...
			objectId = 	(String)requestMap.get("objectIds");
  ...
  */
   ...
   public MapList getAffectedChangeActions(Context context, String[] args) throws Exception {
	...
	public void disconnectLists(Context context, String objectId) throws Exception{
		...
		MapList sObjList = changeOrder.getRelatedObjects(context,
				RELATIONSHIP_OBJECT_ROUTE+","
						+RELATIONSHIP_EC_DISTRIBUTION_LIST,
								TYPE_ROUTE+","
								+TYPE_ROUTE_TEMPLATE+","
								+ChangeConstants.TYPE_MEMBER_LIST,
								null,
								sRelSelectList,
								true,  // getTo
								true,   // getFrom
		...
		public MapList expandProgramCO(Context context,String args[]) throws Exception{
			try
			{
				...
				if(!UIUtil.isNullOrEmpty(objectId)){
					...
					{
						sConnectedItems  = dmObj.getRelatedObjects(context,
								ChangeConstants.RELATIONSHIP_CHANGE_AFFECTED_ITEM,
								QUERY_WILDCARD,
								busSelects,
								null,
								false,
  ...
  /*public StringList getAssigneeCAs(Context context,String personObjectId) throws Exception
  {
    ...
    public MapList getMyChangeTemplates(Context context,String args[]) throws Exception
    {
        ...
        try
        {
            ...
            listCT = DomainObject.findObjects(context,
                    typePattern,    // type filter
                    QUERY_WILDCARD,       // vault filter
		...
		try{
			...
			if(!UIUtil.isNullOrEmpty(objectId)){
				...
				if(dmObj.isKindOf(context, ChangeConstants.TYPE_CHANGE_ACTION)){
					...
					sConnectedItems  = dmObj.getRelatedObjects(context,
							RELATIONSHIP_PART_FAMILY_MEMBER,//check
							QUERY_WILDCARD,
							selectList,
							null,
							false,
...
/**
			...
			if(!COInfoList.isEmpty()){
				...
				while(sItr.hasNext()){
					...
					if (type.equalsIgnoreCase(ChangeConstants.TYPE_CHANGE_ACTION)){
						if((ChangeConstants.STATE_CHANGE_ACTION_PENDING.equalsIgnoreCase(current) || ChangeConstants.STATE_CHANGE_ACTION_PREPARE.equalsIgnoreCase(current) || ChangeConstants.STATE_CHANGE_ACTION_INWORK.equalsIgnoreCase(current) || ChangeConstants.STATE_CHANGE_ACTION_HOLD.equalsIgnoreCase(current)) && (context.getUser().equalsIgnoreCase(owner))){
							sbEditIcon.append("<a href=\"JavaScript:emxTableColumnLinkClick('");
					...
					if(!type.equalsIgnoreCase(ChangeConstants.TYPE_CHANGE_ORDER) && !type.equalsIgnoreCase(ChangeConstants.TYPE_CHANGE_REQUEST) && !type.equalsIgnoreCase(ChangeConstants.TYPE_CHANGE_ACTION)) {
						sbEditIcon.append("-");
						columnVals.add(sbEditIcon.toString());
					}
	...
	public StringList getAllTemplateOIds(Context context, String[] args) throws Exception
	{
		...
		ChangeTemplateList = DomainObject.findObjects(context,
				typePattern,    // type filter
				QUERY_WILDCARD,       // vault filter
	...
	public void connectChangeOrderRelationships(Context context, String args[]) throws Exception
	{
		...
		{
			currentChangeOrderObject.addRelatedObject(context,
					new RelationshipType(sChangeRel),
					false,
					sChangeTemplateOID);
		}
...
public StringList excludeCandidateItems(Context context, String args[])throws Exception
		...
		try
		{
			...
			while (objectItr.hasNext()) {
				...
				if(!ChangeUtil.isNullOrEmpty(strAffectedObjID)) {
					...
					objectIcon.append(UINavigatorUtil.getTypeIconProperty(context, strAffectedObjType));
					strTreeLink = "<a href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?objectId=" + XSSUtil.encodeForHTMLAttribute(context, strAffectedObjID) + "', '800', '575')\"><img border='0' src='../common/images/"+XSSUtil.encodeForHTMLAttribute(context, objectIcon.toString())+"'/>"+XSSUtil.encodeForHTML(context, strAffectedObjName)+"</a>";
					sb.append(strTreeLink);
                    objectIcon.setLength(0);
	...
	public int CheckForAssignees(Context context, String args[]) throws Exception {
		...
		try {
			...
			if (changeActionList.size() > 0) {
				...
				Map objMap;String techAssignee;String SrTechAssignee;
				for(Iterator mapListItr = mapList.iterator();mapListItr.hasNext(); ) {
					objMap = (Map)mapListItr.next();
	...
	public int checkIfAffectedItemAlreadyConnectedToChangeObject(Context context, String args[]) throws Exception {
		...
		try {
			...
			if (ChangeConstants.RELATIONSHIP_CHANGE_AFFECTED_ITEM.equalsIgnoreCase(strRelType)) {
				...
				} else if (strNewRCValue.equalsIgnoreCase(ChangeConstants.FOR_RELEASE) || strNewRCValue.equalsIgnoreCase(ChangeConstants.FOR_OBSOLESCENCE) || strNewRCValue.equalsIgnoreCase(ChangeConstants.FOR_REVISE)) {
	...
	public HashMap FastTrackProcessOptionsRangeValues(Context context,String[] args){
		String [] rangeOptionsStrings = {"EnterpriseChangeMgt.Range.FastTrackProcess.Immediate",
				"EnterpriseChangeMgt.Range.FastTrackProcess.Deferred"};
	...
	public boolean showEffectivityInCOAffectedItems(Context context,String []args) throws Exception {
		/*boolean sReturn          = false;
		HashMap paramMap         = (HashMap) JPO.unpackArgs(args);
		HashMap requestMap       = (HashMap) paramMap.get(REQUEST_MAP);
		...
		if(isECCInstalled)
		{
		       ...
		       Object classObj = cl.newInstance();

		       Method method = cl.getMethod("getEffectivityOnChange", new Class[] {Context.class, String.class});
		       MapList mlObjectExpression = (MapList)method.invoke(classObj, new Object[] {context, objectId});
			   Map mapExpression  = (Map)mlObjectExpression.get(0);
			   String actualValue = (String)mapExpression.get("actualValue");*/
