      ...
      for (int i = 0; i < reqObjects.size(); i++)
      {
         ...
         //PLMxEntityDef[] plmPaths = ooLink.getImplementing();
          ...
          for(int j = 0; allPlmPaths != null && j < allPlmPaths.length; j++)
          {
	         ...
	         /*
	         //------------------------------------
	         //Get the implementED Requirement
	         //------------------------------------
	         PLMxEntityDef[] reqPaths = path; //ooLink.getImplemented();
	         PLMxRefInstanceEntity reqInst = (PLMxRefInstanceEntity) reqPaths[reqPaths.length-1];
	         // BUG 372333/IR A0659289
	         if (reqInst == null) continue;

	         String plmRefId = reqInst.getRefPLMIdentifier();
	         printDebugTrace("plmRefId", plmRefId, debugTrace);
	         //System.out.println(" => Implemented type: " + reqInst.getPLMType());
	         //SpecificationStructure.printIndentedMap(reqInst.getAttributes());

	         // BUG! IR# 642650 - Only implemented Requirements should be returned here,
	         // but it seems that Functions are also returned. So we will ignore them...
	         if (!PLMDictionaryServices.isSpecializationOf(reqInst, RFLPLMREQUIREMENT))
	            continue;

	         // First, get the Matrix ID of the published Requirement object...
	         //StringList plmRefIDs = new StringList(plmRefId);
	         String[] prjReqIDs = coreModeler.convertPLMIDinM1ID(new String[]{plmRefId});
	         String plmReqID = (prjReqIDs == null || prjReqIDs.length == 0? null: prjReqIDs[0]);
	         printDebugTrace("plmReqID", plmReqID, debugTrace);

	         DomainObject plmReqObj = DomainObject.newInstance(emxContext, plmReqID);
			 checkForNull("plmReqObj", plmReqObj, debugTrace);

			 String objId = plmReqObj.getId();
			 String objType = plmReqObj.getInfo(emxContext, DomainConstants.SELECT_TYPE);
			 String objName = plmReqObj.getName();
			 printDebugTrace("objId", objId, debugTrace);
			 printDebugTrace("objType", objType, debugTrace);
			 printDebugTrace("objName", objName, debugTrace);
			*/
