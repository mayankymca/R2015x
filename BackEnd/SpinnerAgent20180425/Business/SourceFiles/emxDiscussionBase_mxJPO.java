    ...
    try
    {
        ...
        if(!rootMessageObjectId.equals(parentObjId))
        {
            ...
            int count;
               try
               {
                  count = Integer.parseInt(rootMsgobj.getAttributeValue(context,
                                                                    ATTRIBUTE_COUNT));
               }
 ...
 */
 ...
 public Map createDiscussionPostProcess(Context context, String[] args) throws Exception {
  ...
  public int checkReplyOnDelete(Context context, String[] args) throws Exception {
  	String fromId = args[0];
  	String toId = args[1];
  	int status = 0;
  	DomainObject messageObject = new DomainObject(toId);
  	Access msgAccess = messageObject.getAccessMask(context);
  	boolean hasRemAccess = AccessUtil.hasContentRemoveAccess(msgAccess);
  	if(!hasRemAccess){
  		String strDeleteDiscussionError = EnoviaResourceBundle.getProperty(context, "emxComponentsStringResource",new Locale(context.getSession().getLanguage()),"emxComponents.Common.DeleteDiscussionHasRepliesAlertMsg");
  		emxContextUtilBase_mxJPO.mqlNotice(context, strDeleteDiscussionError);
  		status = 1;
  	}    	
  	return status;
  }
