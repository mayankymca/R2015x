...
//till here
       ...
       /* personSelect = newBuyerDesk.expandSelect(context,DomainObject.RELATIONSHIP_ASSIGNED_BUYER,DomainObject.TYPE_PERSON,
                                                 selectStmts,new StringList(),true, false, (short)1);
    ...
    /*  while (relItr.next()) {
        projectMemberAttributesTable =  relItr.obj().getTargetData();
        vectProjectMember.addElement((String)projectMemberAttributesTable.get("id"));
      }*/
       while (personIter.hasNext()) {
           vectProjectMember.addElement(personIter.next().getTargetSelectData("id"));
       }
...
//    context.getSession().putValue("error.message", exp.getMessage());
   ...
   //workspace.close(context);
    ...
    try {
      /*expMemberList = projectPerson.expandSelect(context,relPattern.getPattern(),typePattern.getPattern(),
                                                                  selectStmts, new StringList(),true, true, (short)1);
      if ( expMemberList.getRelationships().size() == 0 ) {
        return true;
      } else {
        relItr = new RelationshipWithSelectItr(expMemberList.getRelationships());
        while (relItr.next()) {
          projectMemberAttributesTable =  relItr.obj().getTargetData();
          vectProjectMember.addElement( (String)projectMemberAttributesTable.get("id") );
        }*/
        ...
        /*boGeneric = boWorkspace.expandSelect( context,
          ...
          while(vaultIdItr.hasNext()) {
            ...
            if(vaultId != null && !vaultId.equals("#DENIED!")) {
              ...
              if(vault.containsKey(projectName)){
            	  String strCurrentFoldName = (String)vault.get(projectName);
            	  tempVaultName = strCurrentFoldName +";"+tempVaultName;
...
//    Bug No 312917 - End
  ...
  public Map createWorkspaceProcess(Context context,String[] args) throws Exception
  {
      try{
          ...
          {
              retMap.put("ErrorMessage", EnoviaResourceBundle.getProperty(context, "emxTeamCentralStringResource",
  					context.getLocale(), "emxTeamCentral.Common.Workspace") + " " + strWSName + " " + EnoviaResourceBundle.getProperty(context, "emxTeamCentralStringResource",
  					context.getLocale(), "emxTeamCentral.Common.AlreadyExists"));
  					return retMap;
          }
