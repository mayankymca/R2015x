	...
	protected void normalizeRequirementStructure(Context context,String[] args) throws Exception
    {
        try
        {
            ...
            for(int i=0;i<mapList.size();i++)
            {
                ...
                reqObj.setId(strId);
				SpecificationStructure.normalizeSequenceOrder(context, reqObj, RequirementsUtil.getSubRequirementRelationship(context), false);
				SpecificationStructure.normalizeSequenceOrder(context, reqObj, RequirementsUtil.getDerivedRequirementRelationship(context), false);
}
        }
        catch(Exception ex)
        {
