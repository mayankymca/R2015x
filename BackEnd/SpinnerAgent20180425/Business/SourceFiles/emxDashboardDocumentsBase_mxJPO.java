...
//                sbWBSStructure.insert(0, "<a title='" + sName + "' href='emxTree.jsp?mode=insert&amp;emxSuiteDirectory=programcentral&amp;suiteKey=ProgramCentral&amp;objectId=" + sId + "'>");
    ...
    public String[] getUserDashboardData(Context context, String[] args) throws Exception {
        ...
        for(int i = 0; i < mlDocuments.size(); i++) {
            ...
            if(cOriginated.after(calThreshold)) { mlDocumentsNEW.add(mDocument); }
            ...
            if(iYearNow == iYearOrig) {
                if(iMonthNow == iMonthOrig) { iCounters[1]++; }
                if(iWeekNow  == iWeekOrig)  { iCounters[0]++; }
            }                                
                ...
                if(iYearNow == iYearMod && sDateMod.equals(sDateOrig)==false ) {
                    if(iMonthNow == iMonthMod) { iCounters[3]++; }
                    if(iWeekNow  == iWeekMod)  { iCounters[2]++; }
                }                
        ...
        if(mlDocumentsMOD.size() > 0) {
            ...
            for(int i = 0; i < mlDocumentsMOD.size(); i++) {
                ...
                if(sDate.equals(sDatePrevious)) { iCountMOD++; }
                else {
        ...
        sbInfo1.append("<b>").append(iCounters[0]).append("</b>").append(sInfoPrefix).append("getDocuments&mode=NewWeek&header=emxFramework.String.DocumentsCreatedThisWeek\")'>").append(EnoviaResourceBundle.getProperty(context, "Framework", "emxFramework.String.NEWThisWeek" , sLanguage)).append("</a>");
        sbInfo2.append("<b>").append(iCounters[1]).append("</b>").append(sInfoPrefix).append("getDocuments&mode=NewMonth&header=emxFramework.String.DocumentsCreatedThisMonth\")'>").append(EnoviaResourceBundle.getProperty(context, "Framework", "emxFramework.String.NEWThisMonth" , sLanguage)).append("</a>");
        sbInfo3.append("<b>").append(iCounters[2]).append("</b>").append(sInfoPrefix).append("getDocuments&mode=ModWeek&header=emxFramework.String.DocumentsModifiedThisWeek\")'>").append(EnoviaResourceBundle.getProperty(context, "Framework", "emxFramework.String.MODThisWeek" , sLanguage)).append("</a>");
        sbInfo4.append("<b>").append(iCounters[3]).append("</b>").append(sInfoPrefix).append("getDocuments&mode=ModMonth&header=emxFramework.String.DocumentsModifiedThisMonth\")'>").append(EnoviaResourceBundle.getProperty(context, "Framework", "emxFramework.String.MODThisMonth" , sLanguage)).append("</a>");
        ...
        sbCounter.append("<td onclick='openURLInDetails(\"../common/emxIndentedTable.jsp?table=APPDashboardUserDocuments&program=emxDashboardDocuments:getDocuments&mode=Locked&header=emxFramework.String.LockedDocuments&freezePane=RouteStatus,Name,Title,Actions,NewWindow&suiteKey=Framework\")'");
        sbCounter.append(" class='counterCell ");
        if(mlDocumentsLocked.size() == 0)  { sbCounter.append("grayBright"); }
        else                { sbCounter.append("redBright");  }
        ...
        sbUpdates.append("<td ");
        if(iCountMRU > 0) {
            sbUpdates.append(" onclick='openURLInDetails(\"../common/emxIndentedTable.jsp?table=APPDashboardUserDocuments&program=emxDashboardDocuments:getDocuments&mode=MRU&header=emxFramework.String.MRUDocuments&freezePane=RouteStatus,Name,Title,Actions,NewWindow&suiteKey=Framework\")' ");
