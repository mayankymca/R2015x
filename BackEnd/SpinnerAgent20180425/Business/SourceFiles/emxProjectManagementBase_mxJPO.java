    ...
    */
	...
	try
    {
		...
		String hasModifyOnParent  = (String) typeInfo.get("to["+DomainConstants.RELATIONSHIP_SUBTASK+"].from.current.access[modify]");
    		...
    		String currentState  = (String) typeInfo.get(SELECT_CURRENT);
    		StringList stateList = new StringList();
    		stateList.add(ProgramCentralConstants.STATE_PROJECT_TASK_ACTIVE);
    		stateList.add(ProgramCentralConstants.STATE_PROJECT_TASK_REVIEW);
    		stateList.add(ProgramCentralConstants.STATE_PROJECT_TASK_COMPLETE);
		...
		if("TRUE".equalsIgnoreCase(isKindOfTaskManagement) && !logged_in_user.equalsIgnoreCase(ownerName) && "FALSE".equalsIgnoreCase(hasModifyOnParent)){
    			...
    			strErrorMsg = "emxProgramCentral.Project.NoRightsToDeleteTask";
    			throw new Exception();
    		}else if("TRUE".equalsIgnoreCase(isKindOfTaskManagement)   && stateList.contains(currentState)){
    			strErrorMsg = "emxProgramCentral.Common.UnableToDeleteTasks";
    			throw new Exception();
		}
      ...
      //Modified:27-Dec-2010:s4e:R211 PRG:IR-068141V6R2012
		...
		else
		{
            ...
            utsList.sort(SubtaskRelationship.SELECT_SEQUENCE_ORDER,
           ...
           //END:Commented and Added for IR-226589V6R2014x
            if (utsList.size() > 0)
            {
                ...
                while (itr.hasNext())
                {
                    ...
                    DomainObject dmoTask = DomainObject.newInstance(context, taskId);
                    boolean isProjectSpace = false;
                    isProjectSpace =  dmoTask.isKindOf(context, TYPE_PROJECT_SPACE);
                    if(!isProjectSpace){
                    task.setId(taskId);
        ...
        discussion.deleteMessages(context, (DomainObject)project);
    		...
    		strErrorMsg  = emxProgramCentralUtilBase_mxJPO.getMessage(context,
    				strErrorMsg,
    				null,
    				null,
    				null);
