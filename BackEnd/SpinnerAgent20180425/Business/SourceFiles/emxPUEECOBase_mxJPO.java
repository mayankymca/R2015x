    ...
    */
    private void updateRequestedChange(Context context, String ecoId, DomainObject parentPart) throws Exception {
        ...
        String strRequestedChangeValue  = EnoviaResourceBundle.getProperty(context,
                                                                    "emxFrameworkStringResource",new Locale(strLanguage),"emxFramework.Range.Requested_Change.For_Revise");
    ...
    public int releaseECO(Context context, String[] args) throws Exception
    {
        ...
        String strLanguage = "en";
        String CONST_FOR_RELEASE = EnoviaResourceBundle.getProperty(context,
                                                                    "emxFrameworkStringResource",new Locale(strLanguage),"emxFramework.Range.Requested_Change.For_Release");
        String CONST_FOR_REVISE  = EnoviaResourceBundle.getProperty(context,
                                                                    "emxFrameworkStringResource",new Locale(strLanguage),"emxFramework.Range.Requested_Change.For_Revise");
        ...
        MapList affectedPartIds  = eco.getRelatedObjects(context, RELATIONSHIP_AFFECTED_ITEM,
                                                         DomainConstants.TYPE_PART,slBusSelect,
                                                         slRelSelect,false,true,(short)1, "","");
        ...
        try
        {
	        ...
	        while(affectedItemItr.hasNext()) {
	            ...
	            ebom = affectedPart.getRelatedObjects(context,
	            ...
	            if (CONST_FOR_REVISE.equalsIgnoreCase(requestedChange)) {
	                ...
	                ebomNewRev = newPart.getRelatedObjects(context,
	            ...
	            while (itr.hasNext()) {
	            	...
	            	if (UnresolvedEBOMConstants.RELATIONSHIP_EBOM_PENDING.equals(relType)) {
	            		...
	            		if (CONST_FOR_REVISE.equalsIgnoreCase(requestedChange)) {
	                    	...
	                    	slEBOMChangeHistoryOp = toStringList(map.get("tomid["+UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE_HISTORY+"].attribute["+UnresolvedEBOMConstants.ATTRIBUTE_BOM_OPERATION+"].value"));
	            			...
	            			for(int i=0; i<slEBOMChangeHistoryRels.size(); i++) {
            					...
            					connectObjToRel(context, (String)slEBOMChangeHistoryRels.get(i), newEBOMRelId,UnresolvedEBOMConstants.ATTRIBUTE_BOM_OPERATION,(String)slEBOMChangeHistoryOp.get(i), UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE_HISTORY);
	                    	}
	                        ...
	                        for(int i=0; i<slNEUObjects.size(); i++) {
	                        	connectRelToObj(context, (String)slNEUObjects.get(i), newEBOMRelId,ATTRIBUTE_EFFECTIVIYY_USAGE_INDEX,(String)slNEUIndex.get(i), RELATIONSHIP_NAMED_EFFECTIVITY_USAGE);
	                        ...
	                        for(int i=0; i<slChangeECOIds.size(); i++) {
	                        	if (!ecoId.equals(slChangeECOIds.get(i))) {
	                        		...
	                        		connectObjToRel(context, slChangeECOIds.get(i).toString(), newEBOMRelId, UnresolvedEBOMConstants.ATTRIBUTE_BOM_OPERATION,"Remove", UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE);
	                                ...
	                                // create a list of pending ebom change rels to disconnect from the old rev
	                        		connectObjToRel(context, slChangeECOIds.get(i).toString(), newEBOMRelId,UnresolvedEBOMConstants.ATTRIBUTE_BOM_OPERATION,"Remove", UnresolvedEBOMConstants.RELATIONSHIP_EBOM_CHANGE_HISTORY);
	                                changeRelsDisconnectList.add((String)slChangeRelIds.get(i));
		                            mapEffectivity = new HashMap();
    ...
    */
	public MapList getPUEECOsImpl(Context context, boolean addMultiPart, String[] args) throws Exception {
	   ...
	   private StringList getFeatureOptionEffectivityECOIds(Context context, String modelId) throws Exception {
		   ...
		   /*//selectable for getting CCA / PUE ECO from Model Context.
			String SELECT_CCA_PUEECO_ID_MODEL_CONTEXT = "from[" + REL_CANDIDATE_CONFIG_FEATURES + "].to.from[" + REL_CONFIG_OPTIONS +
			"].tomid[" + EffectivityFramework.RELATIONSHIP_EFFECTIVITY_USAGE + "].from.to[" +
			EffectivityFramework.RELATIONSHIP_NAMED_EFFECTIVITY + "].from[" + types + "].id"; */
	...
	/** This api will be called when CCA OR PUEECO promoted from Create to Define Components. 
	 * api returns 1 if user trying to promote CCA which does not have any affected items connected to it.
	 * @param context ematrix context
	 * @param args
			...
			if (!stateCancelled.equals(nextState)) {
				...
				if ("False".equalsIgnoreCase(affetcedItemRelExits)) {
					emxContextUtilBase_mxJPO.mqlNotice(context, 
																EnoviaResourceBundle.getProperty(context, "emxUnresolvedEBOMStringResource", 
																context.getLocale(), "emxUnresolvedEBOM.Warning.AffectedItemConnected"));
					checkResult = 1;
				}
			}
		}
