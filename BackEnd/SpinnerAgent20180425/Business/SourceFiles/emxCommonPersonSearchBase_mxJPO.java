...
public class emxCommonPersonSearchBase_mxJPO extends emxDomainObject_mxJPO
{
	...
	throws MatrixException
	{
		try {
			...
			for(int itr = 0; itr < strRoles.length; itr++){
				commandTemp = "print role  '"+ strRoles[itr] +"' select person dump |";
				strResult = MqlUtil.mqlCommand(context, commandTemp);
	...
	public StringList includeRiskAssignees(Context context, String[] args)throws MatrixException
	{
		try {
			...
			if(!(project.isKindOf(context, DomainObject.TYPE_PROJECT_SPACE) || project.isKindOf(context, DomainObject.TYPE_PROJECT_CONCEPT))){
