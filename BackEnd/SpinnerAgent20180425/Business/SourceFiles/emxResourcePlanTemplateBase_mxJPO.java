...
public class emxResourcePlanTemplateBase_mxJPO extends emxDomainObjectBase_mxJPO
{
	...
	throws Exception {
    	...
    	try {
			...
			strParenOId = (String)dataMap.get(SELECT_ID);
			DomainObject projecTemplateObject	=	DomainObject.newInstance(context,strParenOId);
			String hasModifyAccess = projecTemplateObject.getInfo(context, "current.access[modify]");
			...
			for (Iterator itrObjects = objectList.iterator(); itrObjects.hasNext();) 
			{
				...
				//Added:23-Dec-2010:s4e:R211 PRG:IR-066371V6R2012 
				StringList relSelect = new StringList();
	            relSelect.add(DomainRelationship.SELECT_FROM_ID);
				strResourcePlanName = (String)mapRowData.get("attribute[" +ResourcePlanTemplate.ATTRIBUTE_RESOURCE_PLAN_TEMPLATE_NAME+"]");
				strColumnName = strResourcePlanName;
                strobjectId = strParenOId;
                //End:Added:23-Dec-2010:s4e:R211 PRG:IR-066371V6R2012
				...
				hasPhaseSubTask = "true".equalsIgnoreCase(strHasPhaseSubTask)?true:false;
					columnName.append("<a href=\"JavaScript:showDialog('");
                                        //Modified: 26-Feb-2011:ms9:IR-069837V6R2012 start
					...
					columnName.append(FrameworkUtil
									.encodeURL("../common/emxIndentedTable.jsp?table=PMCResourcePlanTemplateRequestSummaryTable&" +
											...
											//"&program=emxResourceRequest:getTableResourcePlanTemplateRequestData" +											
		...
		try {
			...
			for(BusinessObjectWithSelectItr itr= new BusinessObjectWithSelectItr(resourceRequestObjWithSelectList); itr.next();)
			{
				...
				for(int i=0; i<slPhaseIdList.size(); i++)
				{
					...
					double nFTE = Task.parseToDouble((String)(bows.getSelectDataList(ResourcePlanTemplate.SELECT_RESOURCE_REQUEST_PHASE_FTE)).get(i));
	...
	public boolean canCreateResourceRequest(Context context,String[] argumentArray) throws MatrixException {
		...
		try {
			...
			String projectTemplateOwner     = projectTemplate.getOwner(context).getName();
			String numberofPeopleUnit       =
					EnoviaResourceBundle.getProperty(context,"emxProgramCentral.ResourcePlan.NumberofPeopleUnit");
			...
			if ((ProgramCentralUtil.isNullString(FTEFilterValue) || FTEFilterValue.equalsIgnoreCase(numberofPeopleUnit))) {
				...
				if (!templatePhaseMapList.isEmpty()) {
					hasAccess = true;
				}
			}
