 ...
 **  @quickReview 17:04:03 ACE2 IR-494888-3DEXPERIENCER2015x: MPI_Project members are not shown in "Resource Name" column 
 **  @quickreview 17:04:06 ACE2 IR-513839-3DEXPERIENCER2015x: Creating a Project from MPI sets Company visibility by default without prompting user for selection
 **  @quickreview 17:04:10 ACE2 Code clean-up: Change roll up order to fix some sub project dependency issues
 **  @quickreview 17:04:10 ACE2 Code clean-up: Change inclusive mapping filter to exclusive
 **  @quickreview 17:04:20 ACE2 IR-515981-3DEXPERIENCER2015x: Estimated and Actual dates of parent task and Project are not getting synced when the changes are made in the child task
 **  @quickReview 17:05:29 ACE2 IR-519220-3DEXPERIENCER2015x: Task added from MS project sets incorrect default value to custom attributes & compare UI keep trying to sync
 * 
 *
 */
...
/**
 * The <code>emxMSFProjectIntegration</code> class represents the JPO for the MS
 * Project integration synchronization mechanism
 */
public class emxMSFProjectIntegration_mxJPO extends ProjectSpace 
{
	/* Fields */
	...
	private ArrayList<String> saveProjects = new ArrayList<String>();
	...
	private Map<String, String> taskColNameBusAttrMap = new HashMap<String,String>(); // Business attrinute updates (Task onky)
	private Map<String, String> taskColNameRelAttrMap = new HashMap<String,String>(); // Relationship attrinute updates (Task onky)
	...
	public emxMSFProjectIntegration_mxJPO () { /* Do nothing */ }
	
	public emxMSFProjectIntegration_mxJPO(Context context, String[] args) throws Exception 
	{
		// instantiate beans
	...
	private String ExecuteMergeProject(Element projectElement, boolean isRootProject, Map<String, String> createdIDs, String userLanguage, String deletedTasks, String deletedProjects, ArrayList<String> invalidResources, boolean parentExists) throws Exception
	{
		...
		if (isRootProject)
		{
			...
			/*else
			{
				for(Map currentCalendar : (List<Map>)project.getDefaultCalendar(context)) 
				{
					project.removeCalendars(context, new StringList(new String[] { (String)currentCalendar.get(SELECT_ID) }));
				}
			}*/
		}
		...
		{
			String projectVisibility = GetElementValue(projectElement, "Visibility");
			...
			if (null != subProjectElement)
			{
				...
				if (null != parentID)
				{
					...
					try
					{
						...
						task.setId(parentID);
						addExistingForMSProject(context, task, subProjectID, parentID, relSubTask);
					}
		...
		try
		{
			...
			else //existing task
			{
				...
				if (parentID != null && parentID.contains("~"))
				{
					...
					task.setId(parentID);
					String nextTaskID = GetElementValue(taskElement, "NextBusId");
					addExistingForMSProject(context, task, taskId , nextTaskID, relSubTask);
				}
	...
	private void SaveProject(String projectId, boolean isRootProject) throws Exception
	{
		...
		for(String saveProjectId : saveProjects)
		{
			task = (Task) DomainObject.newInstance(context, TYPE_TASK, DomainConstants.PROGRAM); 
			task.setId(saveProjectId);
			if(sequenceChanged) { task.reSequence(context, saveProjectId); }
	...
	private void addExistingForMSProject(Context context, Task task, String childTaskId, String atTaskId, RelationshipType subTaskRelationship)
			throws Exception {
		...
		try {
			Map map = task.getNextSequenceInformation(context, task, atTaskId);
			
			Map relAttributes = new HashMap(2);
			relAttributes.put(ATTRIBUTE_SEQUENCE_ORDER, (String) map.get(KEY_SEQ_NUMBER));
			relAttributes.put(SubtaskRelationship.ATTRIBUTE_TASK_WBS, (String) map.get(KEY_WBS_NUMBER));
			
			DomainRelationship.connect(context, (DomainObject) task, subTaskRelationship, DomainObject.newInstance(context, childTaskId))
				.setAttributeValues(context, relAttributes);
