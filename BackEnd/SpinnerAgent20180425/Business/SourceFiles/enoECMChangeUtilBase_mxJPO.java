...
public void affectedItemAddedNotification(Context context, String args[]) throws Exception {
	...
	try
	{
		...
		String sSubjectAddedUnder 	= EnoviaResourceBundle.getProperty(context, ChangeConstants.RESOURCE_BUNDLE_ENTERPRISE_STR, context.getLocale(),"EnterpriseChangeMgt.Message.Subject.AffectedItemAddedUnder");

		String sMessageAddedUnder 	= EnoviaResourceBundle.getProperty(context, ChangeConstants.RESOURCE_BUNDLE_ENTERPRISE_STR, context.getLocale(),"EnterpriseChangeMgt.Message.Body.AffectedItemAddedUnder");
...
public int affectedItemRemovedNotification(Context context, String args[]) throws Exception {
	...
	try
	{
	    ...
	    String sSubject = EnoviaResourceBundle.getProperty(context, ChangeConstants.RESOURCE_BUNDLE_ENTERPRISE_STR, context.getLocale(),"EnterpriseChangeMgt.Message.Subject.AffectedItemRemovedFrom");

	    String sMessage = EnoviaResourceBundle.getProperty(context, ChangeConstants.RESOURCE_BUNDLE_ENTERPRISE_STR, context.getLocale(),"EnterpriseChangeMgt.Message.Body.AffectedItemRemovedFrom");
...
public MapList getConnectedChanges(Context context, String []args) throws Exception {
	  ...
	  while (itr.hasNext()) {
			...
			if(UIUtil.isNotNullAndNotEmpty(changeId) && !changeActions.contains(sCAId)){
				if(!ChangeConstants.STATE_CHANGE_ACTION_PENDING.equalsIgnoreCase(current))mplcaObject.put("disableSelection", "true");
...
public MapList getConnectedLegacyObjects(Context context, String []args)
throws Exception
{
	  ...
	  MapList mplAllRelatedObjects= partObject.getRelatedObjects(context,
			  											QUERY_WILDCARD,
...
//#############################START OF ECM WIDGETS#############################
    ...
    public void getAffectedItemsCountAndLabel(Context context, String[] args) throws Exception {	    
    	try {
    	    ...
    	    new ChangeWidgets().getHTMLForCountAndLabel(context, fieldKey, objectList,
    	    											"EnterpriseChangeMgt.Widget.AffectedItems","EnterpriseChangeMgt.Widget.AffectedItemsMouseOverText",baseURI); 
    	}
    	catch (Exception label) {
