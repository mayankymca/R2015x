...
/**
	 ...
	 * @since R212
	 */
    public Object showGBOMInclusionRuleType(Context context, String[] args)
    throws Exception
    {
		Vector vGBOMInclusionRuleLink = new Vector();
		...
		try {
			...
			LogicalFeature_mxJPO lfeatureJPO = new LogicalFeature_mxJPO(
					context, null);
			MapList mapListDV = lfeatureJPO.getActiveDesignVariants(context,
					arrJPOArgs);
			String strSeleDVAttr = new StringBuffer().append("tomid[").append(
					ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION).append(
					"].from.attribute[").append(
					ConfigurationConstants.ATTRIBUTE_DESIGNVARIANTS)
					.append("]").toString();
			String strSeleIRuleID = new StringBuffer().append("tomid[").append(
					ProductLineConstants.RELATIONSHIP_LEFT_EXPRESSION).append(
					"].from.id").toString();
			...
			for (int i = 0; i < objectList.size(); i++) {
				...
				boolean bIsSimpleInclRule = Part.isSimpleInclusionRule(context,
						argMap).booleanValue();

				String strLanguage = context.getSession().getLanguage();

				if (bIsSimpleInclRule) {
					strRuleType = EnoviaResourceBundle.getProperty(context,SUITE_KEY,
							strLanguage, SIMPLE_INCLUSION_RULE_KEY);
				} else {
					strRuleType = EnoviaResourceBundle.getProperty(context,SUITE_KEY,
							strLanguage,
							"emxProduct.Rule_Type.ComplexInclusionRule");
				}
 ...
 throws Exception {
		...
		String strRuleTypeDisplayInclusion = i18nnow.GetString(
				"emxFrameworkStringResource", strLanguage,
				"emxFramework.Range.Rule_Type.Inclusion");
		String strRuleTypeDisplayExclusion = i18nnow.GetString(
				"emxFrameworkStringResource", strLanguage,
				"emxFramework.Range.Rule_Type.Exclusion");
		for (int i = 0; i < lstobjectList.size(); i++) {
			objectMap = (Map) lstobjectList.get(i);
			String strIncRuleID = "";
		...
		strEndPart2 = "</a>";
			...
			if (strRelId != null && !("".equals(strRelId))
					&& !("null".equals(strRelId))) {
				if (objectMap
						.containsKey(ConfigurationConstants.SELECT_ATTRIBUTE_RULE_TYPE)
						&& ProductLineCommon
								.isNotNull((String) objectMap
										.get(ConfigurationConstants.SELECT_ATTRIBUTE_RULE_TYPE))){
