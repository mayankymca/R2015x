	...
	public Vector getColumnDocumentFileSizeData(Context context,String[] args) throws Exception
	{
		...
		for(BusinessObjectWithSelectItr itr= new BusinessObjectWithSelectItr(folderContentWithSelectList); itr.next();)
		{
			...
			if("true".equalsIgnoreCase(sIsDocumentsType) && "true".equalsIgnoreCase(strIsVersionObject))
			{
				...
				if(ProgramCentralUtil.isNotNullString(strMasterId)&&ProgramCentralUtil.isNotNullString(sChildObjId))
				{
		...
		try {
			...
			if(emxTableRowId!=null)
			{
				...
				if(ProgramCentralUtil.isNullString(parentOId))
				{
					...
					//projectID = parentOId; // commented because while creating subfolder under folder in folder context, projectID is set as folderID  
...
*/
	...
	public Vector getColumnFolderDocumentActionData(Context context,String[]args) throws Exception
	{
		...
		for(int i=0;i<listSize;i++)
		{
			...
			if("true".equalsIgnoreCase(isWorkspaceVault)){
				...
				//Added for special character.
				if(!strBookmarkURL.contains("http://") && !strBookmarkURL.contains("https://")){
			...
			else if("true".equalsIgnoreCase(isDoc) && isVersionObject)
			{
				...
				if ( strFileSize != null && strFileSize != "" && !strFileSize.equals(DomainConstants.EMPTY_STRING ))
				{
			...
			if(selectedObjectIds != null && selectedObjectIds.trim().length()>0){
				...
				while(it1.hasNext()) {
					...
					if(!(STYPE_WORKSPACE_VAULT.equalsIgnoreCase(objType)||STYPE_CONTROLLED_FOLDER.equalsIgnoreCase(objType))){
						...
						output.append("}");
						output.append("else { window.close(); }");  //End Added:PRG:RG6:R212:2-Jun-2011:
			...
			if(selectedObjectIds != null && selectedObjectIds.trim().length()>0){
				...
				while(it1.hasNext()) {
					...
					if(!dObj.isKindOf(context, DomainConstants.TYPE_DOCUMENT)){
						output.append("<script language=\"javascript\">");
						output.append("alert(\"");output.append(strUncommonObjs);output.append("\");");
						output.append("window.parent.close();");
						output.append("</script>");
						//append javascript alert to o/p
						// close the window
						return output.toString();
		...
		try
		{
			...
			for(int i=0; i< size; i++){
				...
				if(objectId==null || is_WorkspaceVault==null || is_ControlledFolder==null || is_VersionedObject==null 
						|| revision==null || is_Document==null){
					needDBCall = true;
				}
			}
			...
			for(int i=0;i<size;i++)
			{
				Map mapRevision = (Map) objectList.get(i);
				if(needDBCall){
					Map infoMap = (Map)objectInfoList.get(i);
					mapRevision.putAll(infoMap);
				}
				...
				if(ProgramCentralUtil.isNotNullString(strObjectId) && ((mChildObjInfo = (Map)mAllObjectInfo.get(strObjectId))!= null))
				{
					...
					if(isControlledFolder || (isDocument && !isVersionObject))
					{
		...
		try {
			...
			if(dObj.isKindOf(context,CommonDocument.TYPE_DOCUMENTS) && ! isVersionObject){ // document type only
				...
				if(!isParentSuperceded){
					...
					//Code for dynamically creating command "AEFLifecycle"
					Map hmCmdFolderProperties = new HashMap();
					hmCmdFolderProperties = createDynamicCommand(context,"AEFLifecycle",uiMenu,1);
					categoryMapList.add(hmCmdFolderProperties);

					//Code for dynamically creating command "PMCDocumentEditActionLink"
					Map hmCmdDocEdit = new HashMap();
					hmCmdDocEdit = createDynamicCommand(context,"PMCDocumentEditActionLink",uiMenu,0);
			...
			if(isVersionObject){   // of type file only
				...
				if(!isParentSuperceded){
					...
					hmCmdFileEdit = createDynamicCommand(context,"PMCDocumentEditActionLink",uiMenu,0);
	...
	private Map createDynamicCommand(Context context,String commandName,UIMenu uiMenu,int flag) throws Exception{
		...
		try{
			...
			if("PMCDocumentCheckInActionLink".equalsIgnoreCase(commandName)){
					label = "emxComponents.Command.UploadFiles";
					strRegSuite = "Components";
				}
	...
	throws MatrixException
	{
		try {
			...
			for(int i=0; i<size; i++){
				...
				String owner = (String)objectInfo.get(DomainObject.SELECT_OWNER);
				
				if(isProjectSpace==null || isProjectTemplate==null || isProjectConcept==null || owner==null){
					needDBCall = true;
				}
			}
			...
			for(int i=0; i< size; i++)
			{
				...
				String strFolderOwner = (String)objectInfo.get(DomainObject.SELECT_OWNER);
				String strState = (String)objectInfo.get(ProgramCentralConstants.SELECT_CURRENT);
				String hasModifyAccess = (String)objectInfo.get("current.access[modify]");
				...
				if(isProject)
					strList.add("false");
				else if("true".equalsIgnoreCase(hasModifyAccess) && (!"Released".equalsIgnoreCase(strState) && !"Obsolete".equalsIgnoreCase(strState)))
					strList.add("true");
				else
					strList.add("false");
			}
	...
	throws MatrixException
	{
		try {
			...
			for(int i=0;i<size;i++){
				...
				String isProjectTemplate = (String)objectInfo.get(ProgramCentralConstants.SELECT_IS_PROJECT_TEMPLATE);
				String isProjectConcept = (String)objectInfo.get(ProgramCentralConstants.SELECT_IS_PROJECT_CONCEPT);
				String owner = (String)objectInfo.get(ProgramCentralConstants.SELECT_OWNER);
				String isControlledFolder = (String)objectInfo.get(ProgramCentralConstants.SELECT_IS_CONTROLLED_FOLDER);	
				
				if(isVersionObject==null ||isDocument==null || isProjectSpace==null|| isProjectTemplate==null 
						|| isProjectConcept==null || owner==null || isControlledFolder==null){
					needDBCall = true;
				}
			}
			...
			if(needDBCall){
				...
				slSelects.add(ProgramCentralConstants.SELECT_IS_PROJECT_TEMPLATE);
				slSelects.add(ProgramCentralConstants.SELECT_IS_PROJECT_CONCEPT);
				slSelects.add(ProgramCentralConstants.SELECT_OWNER);
			...
			for(int i=0;i<size;i++)
			{
				...
				String sIsProjectTemplate = (String)objectInfo.get(ProgramCentralConstants.SELECT_IS_PROJECT_TEMPLATE);
				String sIsProjectConcept = (String)objectInfo.get(ProgramCentralConstants.SELECT_IS_PROJECT_CONCEPT);
				String strFolderOwner = (String)objectInfo.get(ProgramCentralConstants.SELECT_OWNER);
				String strState = (String)objectInfo.get(ProgramCentralConstants.SELECT_CURRENT);
   ...
   //START:16/5/2013:Added for IR-225742V6R2014x 
    ...
    public int triggerAccessTypeModifyAction(Context context, String[] args) throws Exception {
    	...
    	slSelectables.add(DomainConstants.SELECT_KINDOF_WORKSPACE_VAULT);
    	...
    	String iskindOfWorkspaceVault = (String)InfoMap.get(DomainConstants.SELECT_KINDOF_WORKSPACE_VAULT);
	...
	public Vector getCollectionNames(Context context, String[] args) throws Exception {
		try {
			...
			for (Iterator itrObjects = mlObjectInfo.iterator(); itrObjects.hasNext();) {
				...
				if(!isprinterFriendly) {
					strValue = "<a title=\"" + XSSUtil.encodeForXML(context, strName) + "\" href=\"JavaScript:emxTableColumnLinkClick('../common/emxTree.jsp?emxSuiteDirectory="+strSuiteDir+"&amp;jsTreeID="+strNodeId+"&amp;objectId="+strObjectId+"', 800,600, 'false', 'popup', '', 'Normal', 'false')\">" ;
					if("newWindow".equalsIgnoreCase(columnName))
					{
						strValue+= "<img src='../common/images/iconNewWindow.gif' border='0' alt='New Window' />";
					}
					else
					{
    ...
    public StringList columnDragIcon(Context context, String[] args) throws Exception 
    {
    	...
    	for (int i=0; i<objectList.size(); i++) {
    		...
    		if(is_WorkspaceVault == null || is_Document==null || is_VersionAbleObject==null || hasToConnectAccess==null || hasToDisconnectAccess==null){
    			needDBCall = true;
    		}
		}
    	MapList objectInfoList = new MapList();
    ...
    public boolean isVisibleOnMobileForTemplate(Context context , String [] args) throws Exception
    {
    	...
    	if(dmObject.isKindOf(context, ProgramCentralConstants.TYPE_PROJECT_TEMPLATE)) {
			//boolean isVisibleOnMobile = !UINavigatorUtil.isMobile(context);
	...
	public StringList includeDocumentOID(Context context, String []args)throws Exception {
		...
		if(typePattern != null && typePattern.contains(":isVersionObject=False")){
			typePattern = typePattern.replace(":isVersionObject=False","");
		}
