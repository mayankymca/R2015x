...
//		}
    ...
    throws Exception
    {
        ...
        try
        {
            ...
            for (int i=0;i<assessmentList.size();i++) {
                ...
                if(null!=(strAssessmentOwner) && !"".equals(strAssessmentOwner)&& !context.getUser().equals(strAssessmentOwner)) {
                    mpAssessment.put("disableSelection", "true");
                }
            }
        }
        catch (Exception ex)
    ...
    public void createAssessment(Context context, String args[]) throws Exception
    {
        ...
        String modifyQuery = " modify bus Assessment \""+name+"\" \"\" name \""+name+"\" revision \""+rev+"\"";
        MqlUtil.mqlCommand(context, modifyQuery, true);
    ...
    public boolean displayEditCommand(Context context, String args[]) throws Exception
    {
        ...
        if (editFlag)
        {
            ...
            String sOwner = domainObject.getInfo(context, domainObject.SELECT_OWNER);
            if(userId.equals(sOwner)) {
                hasAccess=true;
            }
        }
        return hasAccess;
