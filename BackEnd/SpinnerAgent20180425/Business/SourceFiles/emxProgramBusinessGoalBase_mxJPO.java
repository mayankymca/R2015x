...
public class emxProgramBusinessGoalBase_mxJPO extends emxDomainObject_mxJPO
{
	...
	throws Exception
	{
		...
		try
		{
			...
			for (int i = 0; i < actionListSize; i++)
			{
				...
				String comments = (String) objectMap.get(BusinessGoal.SELECT_DESCRIPTION);
				...
				displayDescription.add(display);
	 ...
	 * @since AEF 10.5
	 */
	public boolean showCommand(Context context, String[] args)
	throws Exception
	{
		try
		{
	...
	public Vector getNameColumn (Context context, String[] args) throws MatrixException
	{
	  ...
	  /**
	   * Exclude Person who has Business Manager Role
	   * 
	   * @param context the eMatrix <code>Context</code> object
	   * @param args
	   * @return StringList  
	   ...
	   */ 
	  @com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
	  public StringList excludePersons(Context context, String[] args)throws  MatrixException
	    {

			MapList mlPersonList = new MapList();
			StringList slFilteredPersons = new StringList();
			Map mpObjectMap= new HashMap();		
			...
			try {
				...
				StringList slSelectables = new StringList(1);
				slSelectables.add(DomainObject.SELECT_ID);				
		        String roleEmployee =   PropertyUtil.getSchemaProperty ( context, "role_BusinessManager" );
		    		        
		        mlPersonList=	findObjects( context,
				...
				slRole.addElement(roleEmployee);
				MapList mlFilteredPersons= filterPersonsBasedOnRole(context, mlPersonList, slRole);
				for (int i = 0; i <  mlFilteredPersons.size(); i++){
					mpObjectMap = (Map) mlFilteredPersons.get(i);
					String strPersons = (String)mpObjectMap.get(DomainObject.SELECT_ID);
					slFilteredPersons.add(strPersons);
				}
			...
			return slFilteredPersons;
	    }
	  /**
	   * Filter Person by given role
	   * 
	   * @param context the eMatrix <code>Context</code> object
	   * @param args
	   * @return MapList  
	  ...
	  public static MapList filterPersonsBasedOnRole(Context context, MapList mlPersons, StringList slRole) throws Exception
		{
			...
			while(itrPerson.hasNext())
		    {
				...
				while(!bHasRole && itrRole.hasNext()) {
					String sRole = (String) itrRole.next(); 
