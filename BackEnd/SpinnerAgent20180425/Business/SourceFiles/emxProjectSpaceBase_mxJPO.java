...
public class emxProjectSpaceBase_mxJPO extends com.matrixone.apps.program.ProjectSpace
{
      ...
      throws Exception
    {
        ...
        String ignoreAccess =
            MqlUtil.mqlCommand(context, "get env global IGNORE_ACCESS");
  ...
  //End:nr2:11-05-2010:PRG:R210:For Project Hold Cancel Highlight
        ...
        String selectState) throws Exception
    {
    	...
    	Program program =
    			(Program) DomainObject.newInstance(context,
        ...
        try
        {
    		...
    		String vaultPattern = PersonUtil.getSearchVaults(context, false ,vaultOption);
          ...
          //End:nr2:11-05-2010:PRG:R210:For Project Hold Cancel Highlight
            ...
            //busWhere +=" && type!=" + ProgramCentralConstants.TYPE_EXPERIMENT; //Added for What if functionality
            ...
            if ((objectId != null) && !"".equals(objectId))
            {
    			...
    			projectList = program.getProjects(context, busSelects, busWhere);
            }
            //ends if
            else
            {
                //busSelects.add(project.SELECT_PROGRAM_NAME);
    			projectList = project.getUserProjects(context, person, busSelects, null,
                        busWhere, relWhere);
            }
            //ends else
        }
    ...
    public Vector getProgram(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            String strProgramName= "";
 		   	...
 		   	String []projectIdArray = new String[size];
 		   	for(int i=0;i<size;i++){
 		   		Map projectIdMap = (Map)objectList.get(i);
 		   		String id = (String)projectIdMap.get(SELECT_ID);
 		   		projectIdArray[i] = id;
 		   	}
            MapList programInfoMaplist =  DomainObject.getInfo(context, projectIdArray,new StringList("to["+RELATIONSHIP_PROGRAM_PROJECT+"].from.name"));
            
            if(programInfoMaplist != null && !programInfoMaplist.isEmpty()) {
            	for(int i=0; i< programInfoMaplist.size(); i++) {
            		Map programMap = (Map)programInfoMaplist.get(i);
            		if(programMap == null || programMap.isEmpty()) {
            			  programs.add("");
            		} else{
                        StringBuffer output = new StringBuffer();
                        String programNamesString = (String)programMap.get("to["+RELATIONSHIP_PROGRAM_PROJECT+"].from.name");
                        String[] programNameArray = programNamesString.split(matrix.db.SelectConstants.cSelectDelimiter); 
                        for(int nCount=0;nCount<programNameArray.length;nCount++) {
                        	strProgramName = programNameArray[nCount];
      ...
      throws Exception
    {
        ...
        try
        {
            ...
            MapList actionList = ProgramCentralUtil.getObjectDetails(context, objIdArr, busSelect);
	   ...
	   long start = System.currentTimeMillis();
        ...
        while (prjIterator.hasNext()){
            ...
            if(!"".equals(phase)){
                ...
                sbDisplayPhase.append("<a href ='").append(phaseURL).append("'>");
            //Added for special character.
        ...
        System.out.println("getCurrentPhase::"+(end-start));
    ...
    public StringList transferTaskAssignments ( Context context , String args[] )
        ...
        if ( selectedIds != null ) {
            ...
            for ( int i=0; i < selectedIds.length; i++ ) {
                ...
                try
                {
                    MqlUtil.mqlCommand(context, "start transaction;",true);
                    ...
                    MqlUtil.mqlCommand(context, "commit transaction;", true);
                }
                catch(MatrixException e)
                {
                    MqlUtil.mqlCommand(context, "abort transaction;", true);
                    isError = true;
            ...
            if(isError)
            {
                ...
                MqlUtil.mqlCommand(context, "notice " + "\""+strTxtMessage+"\"" );
            }
...
//End:nr2:PRG:R210:03-06-:2010:For Project Gate Highlight
        ...
        try {
            ...
            for (Iterator<Map<String,String>> iterator = mpProjectList.iterator(); iterator.hasNext();) {
                ...
                if(existingRelatedProjectIdList.contains(strPtojectId)||
                		...
                		projectState.equalsIgnoreCase(STATE_PROJECT_SPACE_ARCHIVE)
    ...
    public void modifyPrimaryOwnership(Context context, String[] args) throws Exception
    {
        ...
        try
        {
            ...
            objectSelects.add(SELECT_PROJECT_TEMPLATE_TYPE_FROM_PAL);
            objectSelects.add(SELECT_PROJECT_VISIBILITY_FROM_PAL);
            objectSelects.add(SELECT_PROJECT_VISIBILITY);
            Map objectInfo = domainObject.getInfo(context, objectSelects);
            
            boolean isKindOfProjectSpace = Boolean.parseBoolean((String)objectInfo.get(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE));
            boolean isKindOfProjectConcept = Boolean.parseBoolean((String)objectInfo.get(ProgramCentralConstants.SELECT_KINDOF_PROJECT_CONCEPT));
            
            
            
            if (isKindOfProjectSpace || isKindOfProjectConcept)
            {
            	projectVisibility = (String)objectInfo.get(SELECT_PROJECT_VISIBILITY);
            }
            else
            {
                projectVisibility = (String)objectInfo.get(SELECT_PROJECT_VISIBILITY_FROM_PAL);
                strType = (String)objectInfo.get(SELECT_PROJECT_TEMPLATE_TYPE_FROM_PAL);
            ...
            if("Company".equalsIgnoreCase(projectVisibility) && 
					(isKindOfProjectSpace || isKindOfProjectConcept)){
				DomainAccess.createObjectOwnership(context, domainObject.getId(context), defaultOrg, null, "Project Member", DomainAccess.COMMENT_MULTIPLE_OWNERSHIP, true);
			}
            
            if ("Members".equalsIgnoreCase(projectVisibility) && (isKindOfProjectSpace || isKindOfProjectConcept)) {
            ...
            // we need to blank POV for Member visible projects.  Budgets & Benefit never inherit so blank if Company or Member visible project.     
            /*if ((projectVisibility.equalsIgnoreCase("Members")) || (domainObject.isKindOf(context,ProgramCentralConstants.TYPE_FINANCIAL_ITEM))) {
            	if(!strType.equalsIgnoreCase(DomainObject.TYPE_PROJECT_TEMPLATE)){
            		domainObject.removePrimaryOwnership(context);
            	}
			}
			else
			{
				defaultOrg=PersonUtil.getDefaultOrganization(context, context.getUser());
				defaultProj=PersonUtil.getDefaultProject(context, context.getUser());
				domainObject.setPrimaryOwnership(context,defaultProj,defaultOrg);
  	...
  	public String getImportField(Context context,String[]args)throws Exception
  	{
  		...
  		sb.append("<td >");
  		sb.append("<input type=\"file\" id= \"FileFullPath\" name=\"FilePath\" size=\"18\" onChange=\"javascript=getFileDetails()\"/>");
  		sb.append("</td>");
  	...
  	public MapList getProjectImportViewDynamicTable(Context context, String[] args) throws Exception
  	{
  		...
  		for(int i=0;i<importedFileHeaderList.size();i++){
			...
			columnMap.put("label",displayHeaderName);
  			...
  			if(headerName.equalsIgnoreCase("Level")){
  				settingMap.put("Sortable","true");
  	...
  	public StringList getQuestionTaskResponseRangeValues(Context context,String[]args)throws Exception
  	{
  		...
  		for(int i=0;i<objectList.size();i++){
  			...
  			sb.append(actualFalse);
  			sb.append("</option>");

  			sb.append("<option value=\""+questionId+"|TRUE\">");
  			sb.append(actualTrue);
  			sb.append("</option>");
    ...
    public boolean isNotFromProgram(Context context, String[] args) throws Exception {
    	return !isFromProgram(context, args);
    }
    ...
    /**
     * It returns true if target location is not Slidein with edit mode
     * 
     * @param context the ENOVIA <code>Context</code> object.
     * @param args request arguments
     * @return true if form target location is not Slidein with edit mode
     * @throws Exception if operation fails. 
     */
