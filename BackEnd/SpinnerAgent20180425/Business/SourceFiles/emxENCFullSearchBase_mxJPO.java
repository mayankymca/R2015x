...
*/
	...
	/*public StringList excludeOIDAffectedItems(Context context, String[] args)
			throws Exception
		{
...

		{
			...
			{
				String strFieldType = (String) itrFields.next();

				int intIndex = strFieldType.indexOf("TYPES=");

				if (intIndex != -1)
				{
					String strSubTypes = strFieldType.substring(intIndex + 6);
					...
					{
						String strSubType = (String) itrTypes.next();
						strSubType = PropertyUtil.getSchemaProperty(context,strSubType);

						if (strTempSubTypes.length() == 0)
						{
							strTempSubTypes = strSubType;
						}
						else
						{
							strTempSubTypes = strTempSubTypes + "," + strSubType;
						}
					...
					if (strTempSubTypes.length() > 0)
					{
						strTypePattern = strTempSubTypes;
					}
				...
				StringList strlAffectedItemselects = new StringList(2);
				strlAffectedItemselects.add(DomainConstants.SELECT_ID);
				strlAffectedItemselects.add("to[" + sRelDesignResponsibility + "].from.id");
				...
				MapList totalresultList = DomainObject.findObjects(context,
																 ...
																 null,
																 true,
																 strlAffectedItemselects,
																 (short) 0);

				Iterator itrAffectedItems = totalresultList.iterator();
     			String strAffectedItemRDOId = "";

				while (itrAffectedItems.hasNext())
				{
					Map mapAffectedItem = (Map) itrAffectedItems.next();
					String strAffectedItemId = (String) mapAffectedItem.get(DomainConstants.SELECT_ID);
					try
					{
						strAffectedItemRDOId = (String) mapAffectedItem.get("to[" + sRelDesignResponsibility + "].from.id");
					}
					catch (Exception ex)
					{
						strAffectedItemRDOId = (String) ((StringList) mapAffectedItem.get("to[" + sRelDesignResponsibility + "].from.id")).get(0);
					}
				...
				{
					strlExcludeOIDAffectedItems.add(strAffectedItemId);
				}
				else
				{
					Access access=new Access();
					BusinessObject boChange = new BusinessObject(strAffectedItemId);
					access = boChange.getAccessMask(context);
			...
			MapList mapListAffectedItems = doECO.getRelatedObjects( context,
										 PropertyUtil.getSchemaProperty(context,"relationship_AffectedItem") + "," + PropertyUtil.getSchemaProperty(context,"relationship_NewPartPartRevision") + "," + PropertyUtil.getSchemaProperty(context,"relationship_NewSpecificationSpecificationRevision") + "," + PropertyUtil.getSchemaProperty(context,"relationship_RequestSpecificationRevision") + "," + PropertyUtil.getSchemaProperty(context,"relationship_RequestPartRevision") + "," + PropertyUtil.getSchemaProperty(context,"relationship_RequestPartObsolescence"),
										strTypePattern,
										 objectSelects,
										 null,
			...
			while (itrRelatedAffectedItems.hasNext())
			{
				Map mapChangeObj = (Map) itrRelatedAffectedItems.next();
