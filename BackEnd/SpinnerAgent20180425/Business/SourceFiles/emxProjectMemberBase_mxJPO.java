        ...
        try
        {
            ...
            if (emxTableRowId != null)
            {
                ...
                if (members.size() > 0)
                {
                    ...
                    MqlUtil.mqlCommand(context, "notice '" + usersAddedMsg + "'");
                }
                else{
...
//      Added for the Bug 310681 End
    ...
    throws Exception
    {
        ...
        String objectId = (String) paramMap.get("objectId");
        DomainObject dom = DomainObject.newInstance(context, objectId);
        String symval=PropertyUtil.getSchemaProperty(context,"attribute_RemainingEffort");
        String val = dom.getAttributeValue(context,symval);
        StringBuffer returnString= new StringBuffer();
        String reportFormat = (String) requestMap.get("reportFormat");
        double dVal = Task.parseToDouble(val);
        if(dVal < 0)
        {
            dVal = 0;
        ...
        if(strMode == null)
        {
            if(reportFormat!=null && "CSV".equalsIgnoreCase(reportFormat)){
                returnString.append(XSSUtil.encodeForHTML(context,val));
            }
            else
            {
            returnString.append(XSSUtil.encodeForHTML(context,val));
            returnString.append("<input type=\"hidden\" name=\"remain\" value=\""+XSSUtil.encodeForHTML(context,val)+"\">");
        }
        }
        else
        {
            returnString.append("<input type=\"text\" name=\"RemainingEffort\" readonly value=\""+XSSUtil.encodeForHTML(context,val)+"\" onChange=\"javascript:updateOriginalRemain()\">");
            returnString.append("<script language=\"javascript\">");
            returnString.append("function updateOriginalRemain() {");
            returnString.append("var form_obj = document.editDataForm;");
            returnString.append("form_obj.originalremainingEffort.value = form_obj.RemainingEffort.value; } </script>");

        }
      if(reportFormat == null){
        returnString.append("<input type=\"hidden\" name=\"originalremainingEffort\" value=\""+XSSUtil.encodeForHTML(context,val)+"\">");
        }
        return returnString.toString();
    }
      ...
      throws Exception
    {
        ...
        try
        {
            ...
            {
                String ancestors = MqlUtil.mqlCommand(context, "print role \"" + contextRole + "\" select role.ancestor dump |");
...
//      }
    ...
    throws MatrixException{
        try{
            ...
            //Get Ownership details of the project
            MapList ownershipAccess = ProjectSpace.getOwnershipAccess(context, projectId);
            String ctxUser = context.getUser();
            for (Iterator itrAccess = ownershipAccess.iterator(); itrAccess.hasNext();) {
                Map projectOwnershipInfo = (Map) itrAccess.next();
                String projectMember = (String)projectOwnershipInfo.get(ProjectSpace.KEY_PERSON_NAME);
                //if context is a project member and has Project Lead access in the project, the display the grid.
                if(ctxUser.equals(projectMember)){
                    String access = (String)projectOwnershipInfo.get(DomainAccess.KEY_ACCESS_GRANTED);
                    if(ProgramCentralConstants.PROJECT_ACCESS_PROJECT_LEAD.equals(access)){
                        return true;
                    }
                }
     ...
     * @throws Exception
     */
	public void updateAccessValue(Context context, String[] args)
			throws Exception {
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		HashMap paramMap = (HashMap) programMap.get("paramMap");
		String objectId = (String) paramMap.get("objectId");
