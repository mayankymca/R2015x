			...
			for (int i = 0; i < listSize; i++) {
				...
				while (iterator.hasNext()) {
					...
					String relInfo = MqlUtil.mqlCommand(context,
										  "print connection $1 select $2 $3 $4 $5 $6 $7 $8 $9 $10",
										  relId, "name", "from", "to", "attribute.value", "interface", "tomid[EBOM Change].id", 
										  "tomid[EBOM Change].attribute.value", "frommid.to.id", "frommid.to.attribute.value");

					isContextChangeCase = MqlUtil.mqlCommand(context,
										  "print connection " + relId + " select frommid["+ RELATIONSHIP_NAMED_EFFECTIVITY_USAGE+ "] dump");
