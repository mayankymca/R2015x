 ...
 */
 ...
 public MapList getProductDerivations(Context context, String args[]) throws Exception
 {
     ...
     // If User has no Read Access then empty table will be displayed.
     if(ProductLineCommon.isNotNull(strRootId)&& !"TRUE".equalsIgnoreCase(DomainObject.newInstance(context, strRootId).getInfo(context, "current.access[read]"))) {
    	 return mlDerivations; 
     }
     // Get the consolidated list of Main Derivations, including the root.
     mlDerivations = Product.getMainDerivations(context, strRootId);
...
/**
	 * This is the delete check trigger method on model to check
     * whether context model has any product(s) associated with it.
	 *
	 * @param context the eMatrix <code>Context</code> object
	 * @param args  holds model's object ID
	...
	public int checkIfModelHasProducts(Context context, String args[]) throws FrameworkException{
		...
		try{
			...
			MapList mpObjectList = dmObj.getRelatedObjects(context, ProductLineConstants.RELATIONSHIP_PRODUCTS,
					ProductLineConstants.TYPE_PRODUCTS, objectSelects, relSelects, false, true,
					(short) 0,DomainConstants.EMPTY_STRING,DomainConstants.EMPTY_STRING, 0);
