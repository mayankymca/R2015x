...
public class emxRMTDashBoardBase_mxJPO{  
    ...
    public MapList retrieveRequirementsDashboardItems(Context context, String[] args, StringList busSelects, String sRelationships, String sTypes, String sFilter, Map mFilter) throws Exception {    
        ...
        if(sOID.equals("")) {
            mlResults = DomainObject.findObjects(context, "Requirement", "eService Production", "(owner == '" + context.getUser() + "') && " + sFilter, busSelects);            
	...
	private JSONArray getTestCaseInformation(Context context, String objectId) throws FrameworkException{
		String sTypes           = "*";   
        String sRelationships   = RequirementsUtil.getSubRequirementRelationship(context) + ","
        						+ RequirementsUtil.getSpecStructureRelationship(context) + ","
        						+ RequirementsUtil.getRequirementValidationRelationship(context);  
        StringList busSelects = new StringList();
        ...
        for(int i=0; i<mlRequirements.size();i++){
        	...
        	if(type.equalsIgnoreCase(RequirementsUtil.getTestCaseType(context))){
	...
	private JSONArray getTimeLineInformation(Context context, String objectId) throws ParseException, MatrixException{
        ...
        busSelects.add(DomainConstants.SELECT_TYPE);  
        busSelects.add(DomainConstants.SELECT_CURRENT);
        busSelects.add("state[Propose].start");
        busSelects.add("state[Approve].start");
        ...
        if(mlRequirements.size() > 0) {
            ...
            for(int i=0; i<mlRequirements.size();i++){
            	...
            	String sStart0              = (String) mlRequirement.get("state[Propose].start");
                String sStart1              = (String)mlRequirement.get("state[Approve].start");
                String sStart2              = (String) mlRequirement.get("state[Validate].start");
                String sStart3              = (String) mlRequirement.get("state[Review].start");
	...
	private JSONArray getDerivedRequirementsStatus(Context context, String objectIdList,String direction) throws NumberFormatException, MatrixException{
		...
		result[1] = 0;
		String sTypes           = "*";   
        String sRelationships   = RequirementsUtil.getDerivedRequirementRelationship(context);  
        StringList busSelects = new StringList();
        StringList relSelects = new StringList();
        busSelects.add(DomainConstants.SELECT_ID);
        busSelects.add(DomainConstants.SELECT_TYPE);  
	...
	private JSONObject getSubRequirements(Context context, String objectIdList) throws NumberFormatException, MatrixException{
		...
		String sTypes           = RequirementsUtil.getRequirementType(context);   
		String sRelationships   = RequirementsUtil.getSubRequirementRelationship(context);  
        StringList busSelects = new StringList();
        StringList relSelects = new StringList();
        busSelects.add(DomainConstants.SELECT_ID);
	...
	private JSONArray getTestCaseValues(Context context, String objectList) throws MatrixException{
		...
		String sTypes           = "*";   
        String sRelationships   = RequirementsUtil.getRequirementValidationRelationship(context);  
        StringList busSelects = new StringList();
        StringList relSelects = new StringList();
        busSelects.add(DomainConstants.SELECT_ID);
        busSelects.add(DomainConstants.SELECT_TYPE);  
	...
	private JSONArray getPLMParameterValues(Context context, String objectList) throws MatrixException{
		...
		String sTypes           = "*";   
        String sRelationships   = RequirementsUtil.getParameterUsageRelationship(context)+',';
        sRelationships   = RequirementsUtil.getParameterAggregationRelationship(context);
        StringList busSelects = new StringList();
        StringList relSelects = new StringList();
        busSelects.add(DomainConstants.SELECT_ID);
        //busSelects.add(DomainConstants.SELECT_TYPE);  
