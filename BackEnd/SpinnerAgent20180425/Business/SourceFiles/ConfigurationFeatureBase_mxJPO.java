...
public class ConfigurationFeatureBase_mxJPO extends emxDomainObject_mxJPO {
	...
	* @since FTR R213
	            ...
	            if (strComparable != null
	                ...
	                strBuf
	                        .append("<input id=\"CompareBy\" type=\"checkbox\" "
	                                ...
	                                + XSSUtil.encodeForHTMLAttribute(context, sStrResourceValue)
	                                + "\"  />"
	                                + XSSUtil.encodeForXML(context, sStrResourceValue));
	                strBuf
	...
	/**
	 * This program used to render value in Table
	 * FTRProductContextGBOMDesignChoicesTable-- Feature selection Table in used
	 * while changing DV
	 *
	 * @param context
	 * @param args
	 ...
	 */
	@com.matrixone.apps.framework.ui.ProgramCallable
	public MapList getRangeValuesForDesignVariant(Context context, String[] args)
			throws Exception {
		...
		try {
			...
			String strContextProdId = (String) programMap
			.get("strContextProdObjId");
			String strFeatureId = (String) programMap.get("strFeatureObjId");
			String strNameValue = (String) programMap
			.get("FTRGBOMDVNameCustomFilterCommand");
			String strLimitValue = (String) programMap
			.get("FTRGBOMDVLimitCustomFilterCommand");
			...
			if ((strNameValue != null && !"".equalsIgnoreCase(strNameValue)) ){//|| "true".equalsIgnoreCase(isOnLoad)){
				...
				if ("ConfigurationOption".equals(strMrktingChoiceValue)){//&& ("false".equalsIgnoreCase(isOnLoad))) {
					...
					objSelects.addElement("attribute["
					...
					ConfigurationUtil confUtil = new ConfigurationUtil(
							strDesignVariantId);
					mapBusIds= confUtil.getObjectStructure(context, strObjPattern,
							strRelPattern, objSelects, relSelects, false, true,
							sRecurse, 0, strWhere,
							DomainConstants.EMPTY_STRING, DomainObject.FILTER_STR_AND_ITEM,
							filterExpression);
					//------------------------------------------------------
					...
					for (int i = 0; i < mapBusIds.size(); i++) {
						...
						for (int cnt = 0; cnt < commonGroupList.size(); cnt++) {
							...
							StringList confOptionsRels = (StringList) commonGroupsMap
							...
							for (int j = 0; j < confOptionsRels.size(); j++) {
								...
								if (strGlobalCG_Current != null
										&& strGlobalCG_Current
												.equalsIgnoreCase("Yes")
										&& curentRelID
												.equalsIgnoreCase(confOptionInCG)
										|| strGlobalCG_Current != null
										&& strGlobalCG_Current
												.equalsIgnoreCase("No")
										&& curentRelID
												.equalsIgnoreCase(confOptionInCG)
										&& prdId
												.equalsIgnoreCase(strContextProdId)) {
									mapBusIds.remove(i--);
									check = true;
									break;
								}
					...
					for (int finalcount = 0; finalcount < mapBusIds.size(); finalcount++) {
						...
						tempMap.put(DomainConstants.SELECT_ID, (String) htTempTable
								.get(DomainObject.SELECT_ID));
						tempMap.put(DomainConstants.SELECT_RELATIONSHIP_ID,
								(String) htTempTable
								.get(DomainRelationship.SELECT_ID));
						tempMap.put(DomainObject.SELECT_NAME, (String) htTempTable
								.get(DomainObject.SELECT_NAME));
						tempMap.put(DomainObject.SELECT_CURRENT, policy+"."+state);
						tempMap.put(DomainObject.SELECT_TYPE, (String) htTempTable
						...
						.put(
								...
								.get("attribute["
										+ ConfigurationConstants.ATTRIBUTE_DISPLAY_NAME
										+ "]"));
						tempMap.put(DomainObject.SELECT_REVISION, (String) htTempTable
								.get(DomainObject.SELECT_REVISION));
						tempMap.put(DomainObject.SELECT_LEVEL, (String) htTempTable.get("level"));
						tempMap.put("relationship", (String) htTempTable
								.get("relationship"));
...
/**
			    ...
			    /**
				    ...
				    * @since R211
				    */
				   public StringList excludeConnectedFeatureOptionsInCG(Context context, String [] args)
				   throws FrameworkException
				   {
	 ...
	 * @return StringList- consisting of the object ids to be excluded from the
	 *         Search Results
	 * @throws Exception
	 *             if the operation fails
	 * @exclude            
	...
	@com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
	public StringList excludeDesignVariantOptions(Context context, String[] args)
			throws Exception {
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		...
		DomainRelationship designVariantRel = new DomainRelationship(
				strVariesBYConnectionId);
		String isGlobalCommonGroup = designVariantRel.getAttributeValue(
				context, ConfigurationConstants.ATTRIBUTE_GLOBAL_COMMON_GROUP);
		// MapList
		// commonGroupList=commonValues.getCommonGroupList(context,strDesignVariantOptionId,strDesignVariantId);
		MapList commonGroupList = commonValues.getCommonGroups(context,
				strLFId, strDesignVariantId);
		...
		if (isGlobalCommonGroup.equals("Yes")) {
			...
			for (int i = 0; i < commonGroupList.size(); i++) {
				...
				for (int j = 0; j < cgValues.size(); j++) {
					...
					MapList resultList = DomainRelationship.getInfo(context,
							relIdList, slRelSelects);
					Map mpREToDetails = (Map) resultList.get(0);
					String strFeatureId = mpREToDetails.get("torel.to.id")
							.toString();
			...
			for (int i = 0; i < commonGroupList.size(); i++) {
				...
				temp = (Map) commonGroupList.get(i);
				StringList cgValues = (StringList) temp.get("Common Values");
				String strCommmonGroupContext = temp.get("Context Prod Id")
						.toString();
				if (strCommmonGroupContext.equals(strCurentContext)) {
					for (int j = 0; j < cgValues.size(); j++) {
						String strCommonValueId = (String) cgValues.get(j);
						String[] relIdList = new String[] { strCommonValueId };
						MapList resultList = DomainRelationship.getInfo(
								context, relIdList, slRelSelects);
						Map mpREToDetails = (Map) resultList.get(0);
						String strFeatureId = mpREToDetails.get("torel.to.id")
								.toString();
						featurestoExcludeList.add(strFeatureId);
	    ...
	    @com.matrixone.apps.framework.ui.ProgramCallable
	     ...
	     public int checkAccessForSettingEffectivity(Context context, String [] args) throws Exception
	     {
			 ...
			 if(_attrCFFList.contains(attrName))
			 {
				...
				//skip the parent's FrozenState check if feature is released via Engineering Change and the EC is in Complete state and requested Change is "For Obsolescence"
				 boolean bSkipForEC = false;
				 String sECInfo = PropertyUtil.getGlobalRPEValue(context, "EC_INFO_REQ_CHANGE_DATA");
				 if(sECInfo != null && sECInfo.length() > 0){
					 StringList slECInfo = FrameworkUtil.split(sECInfo, "|");
                     System.out.println("!!!slECInfo!!!: " + slECInfo);
					 if(slECInfo != null && slECInfo.size() > 0){
						 String sECId = (String)slECInfo.get(0);
						 String sECState = (String)slECInfo.get(1);
						 String sReqChange = (String)slECInfo.get(2);
						 if(DomainObject.newInstance(context, sECId).isKindOf(context, PropertyUtil.getSchemaProperty(context, "type_EngineeringChange")) &&
							"Complete".equals(sECState) && 
							"For Obsolescence".equals(sReqChange)){
							 bSkipForEC = true;
						 }
					 }
                     
                     PropertyUtil.setGlobalRPEValue(context, "EC_INFO_REQ_CHANGE_DATA","");
				 }
                 				 
				 if(!bSkipForEC){
					 if(ConfigurationUtil.isFrozenState(context, fromObjectID))
