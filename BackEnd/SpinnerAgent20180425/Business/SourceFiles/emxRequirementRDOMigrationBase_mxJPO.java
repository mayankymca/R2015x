    ...
    /**
	   ...
	   public void migrateCommandAccess(Context context, String[] args) throws Exception
      {
		...
		try{
		  ...
		  /**
	       * This method grant license to view commands
	       *
	       * @param context the eMatrix <code>Context</code> object
	       * @returns nothing
	       * @throws Exception if the operation fails
			...
			try{
				...
				while(CmdItr.hasNext())
				{
					...
					for (int i = 1 ; i < nbrUser ; i ++)
					{
						...
						{
							result = MqlUtil.mqlCommand(context, "modify command $1 add setting $2 $3",true,strCmdName,"Licensed Product", "ENO_RMF_TP,ENOBUPS_AP");
