...
import com.matrixone.jsystem.util.StringUtils;
		...
		public void promoteConnectedCRToCompleteState(Context context,String args[]) throws Exception {
			...
			try {
				...
				if (!bFlag) {
					...
					emxNotificationUtilBase_mxJPO.sendNotification(context,strChangeRequestId, new StringList(strCROwner), ccList,bccList, SubjectKey, MessageKey, lstAttachments,suiteKey, null, null, null);
				    }
				}
			} catch (Exception Ex) {
				Ex.printStackTrace();
	  ...
	  try
        {
		  ...
		  String whereclause = "(policy == \"" + ChangeConstants.FASTTRACK_CHANGE + "\") && (current == \"" + ChangeConstants.STATE_FORMALCHANGE_PREPARE + "\") && ((to[" + ChangeConstants.RELATIONSHIP_CHANGE_ORDER + "].from.id == \"" + crId +"\") || (to[" + ChangeConstants.RELATIONSHIP_CHANGE_ORDER + "]== \"" + isConnected +"\"))";
	...
	public MapList getChangeOrders(Context context, String strChangeRequestId)throws Exception
	{
		...
		return getRelatedObjects(context,
				ChangeConstants.RELATIONSHIP_CHANGE_ORDER,
				ChangeConstants.TYPE_CHANGE_ORDER,
				slObjectSelect,
				slRelSelect,
				false,
