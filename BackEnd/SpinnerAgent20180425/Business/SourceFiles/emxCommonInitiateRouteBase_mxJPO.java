...
//End: Resume Process Implementation
        ...
        try {
            ...
            String mqlret = MqlUtil.mqlCommand(context, "print bus \"" + sType
                    + "\" \"" + sName + "\" \"" + sRev
                    + "\" select id owner dump | ");
            ...
            while (bGreaterSeqNoFound == 1) {
                ...
                for (int i = 0; i < nRoutedNodeObjectsSize; i++) {
                    ...
                    if (routeSequence == sRouteSequenceValue) {
                        ...
                        sRouteNodeId = (String) objectMap
                                .get(DomainConstants.SELECT_RELATIONSHIP_ID);
                        sRouteNodePhysicalId = (String) objectMap
                        .get("physicalid");
                        ...
                        sRPERev = MqlUtil.mqlCommand(context,
                                "get env REVISION");
                        String Vault = MqlUtil.mqlCommand(context,
                                "get env VAULT");
                        MqlUtil.mqlCommand(context,
                                "unset env global MX_TREE_MENU");
                        // Start: Resume Process implementation
                        ...
                        if(UIUtil.isNotNullAndNotEmpty(sRouteNodePhysicalId)&& mlOldTasks.size() == 0){
                        	mlOldTasks = Route.getRelatedObjects(context,
                                    sRelRouteTask, DomainObject.TYPE_INBOX_TASK,
                                    slTaskSelect, null, true, false, (short) 1,
                                    "attribute[" + sAttRouteNodeID + "]=='"
                                            + sRouteNodePhysicalId + "'", "");
                        }
                        ...
                        if (mlOldTasks.size() > 0) {
                            ...
                            // Create a Inbox Task
                            strMQL = "execute program eServicecommonNumberGenerator.tcl '"
                                    + typeTask
                                    + "' '' '' 'Null' '"
                                    + Vault
                                    + "'";
                            String strInboxRes = MqlUtil.mqlCommand(context,
                                    strMQL, true);

                            StringList slTokens = FrameworkUtil.split(
                                    strInboxRes, "|");
                            String sInboxErrorCode = ((String) slTokens.get(0))
                                    .trim();
...
//                      # 370487 set the originator value of task 
                        ...
                        if (!"".equals(sAbsenceDelegate)
                            ...
                            if (absenceMap.size() != 0) {
                                ...
                                for (int i1 = 0; i1 < sRouteObjectsMaplist
                                    ...
                                    lGrantor = MqlUtil.mqlCommand(context,
                                            "print bus " + sObjectId
                                                    + " select grantee grantor dump |");
                                    ...
                                    String output = MqlUtil
                                            .mqlCommand(
                                                    context,
                                                    "print bus "
                                                            + sObjectId
                                                            + " select grantee granteeaccess dump |");
                                ...
                                for (int i2 = 0; i2 < lGrants.size(); i2++) {
                                    HashMap grantMap = (HashMap) lGrants
                                            .get(i2);
                                    sObjectId = (String) grantMap
                                            .get("sObjectId");

                                    lGrantor = (String) grantMap
                                            .get("lGrantor");

                                    lGrantee = (String) grantMap
                                            .get("lGrantee");

                                    lGranteeAccess = (String) grantMap
                                            .get("lGranteeAccess");
                                    // modified for 307078
                                    if ((lGrantor
                                        ...
                                        // till here
                                        MqlUtil.mqlCommand(context,
                                                "push context user '" + sRouteDelegationGrantor + "';modify bus " + sObjectId
                                                        + " grant '"
                                                        + sDelegatorName
                                                        + "' access "
                        ...
                        if (sTitle == null || sTitle.equals("null")
                                || sTitle.equals("")) {
                            sTitle = inboxTask.getInfo(context,
                        ...
                        if ((sRPEType.equals(sInboxTaskType))
                            ...
                            String mqlReturn = MqlUtil.mqlCommand(context,
                                    "expand bus '" + sRPEType + "' '"
                                            + sRPEName + "' '" + sRPERev
                                            + "' from relationship '"
                                            + sRelProjectTask + "' dump | ");
