 ...
 * 
 * 
 * @quickreview JX5		14:11:28 		IR-332766-3DEXPERIENCER2015x : Object linked as covered or derived requirement are getting displayed in search with in context 
 * @quickreview LX6		15:05:27 		IR-350188-3DEXPERIENCER2015x : Search with in context  displays incorrect result. 
 */
...
public class emxRequirementSearchBase_mxJPO extends emxProductSearch_mxJPO
{
	...
	throws Exception
	{
		...
		if (bLatestRevision) {
			...
			sbWhereExp.append(SYMB_EQUAL);
			sbWhereExp.append(new Policy(RequirementsUtil.getRequirementPolicy(context)).hasMajorSequence(context) ? "majorid.lastmajorid.bestsofar.majorrevision" : "last");
			sbWhereExp.append(SYMB_CLOSE_PARAN);
		}
		...
		if (  strPriority!=null && (!strPriority.equals(SYMB_WILD)) && (!strPriority.equals("")) ) {
			...
			sbWhereExp.append(RequirementsUtil.getPriorityAttribute(context));
	...
	throws Exception
	{
		String strType = RequirementsUtil.getUseCaseType(context);
	...
	throws Exception
	{
		return getStates(context, RequirementsUtil.getUseCaseType(context));
	}
	...
	throws Exception
	{
		return getStates(context, RequirementsUtil.getRequirementType(context));
	}
	...
	throws Exception
	{
		...
		sbWhereExp.append(RequirementsUtil.getCandidateItemRelationship(context));
		...
		sbWhereExp.append(SYMB_OPEN_PARAN);
		sbWhereExp.append("!('to[");
		sbWhereExp.append(RequirementsUtil.getCommittedItemRelationship(context));
		sbWhereExp.append("].from.");
		sbWhereExp.append(DomainConstants.SELECT_ID);
		sbWhereExp.append("'==");
		sbWhereExp.append(strObjectId);
	...
	throws Exception
	{
		...
		if (strTitle != null && !strTitle.equals(SYMB_WILD) && !strTitle.equals(""))
		{
			...
			String attName = SYMB_ATTRIBUTE + SYMB_OPEN_BRACKET + RequirementsUtil.getTitleAttribute(context) + SYMB_CLOSE_BRACKET;
	...
	throws Exception
	{
		return getStates(context, RequirementsUtil.getDecisionType(context));
	}
	...
	throws Exception
	{
		return getStates(context, RequirementsUtil.getChapterType(context));
	}
	...
	throws Exception
	{
		return getStates(context, RequirementsUtil.getCommentType(context));
	}
	...
	throws Exception
	{
		...
		if ("RMTSpecificationSearchCommand".equals(strCommandName) || "RMTSpecificationSearchImport".equals(strCommandName)
			...
			sbWhereExp.append(RequirementsUtil.getVersionPolicy(context));
			sbWhereExp.append(SYMB_CLOSE_PARAN);
		}
						...
						if (strActualValue != null) {
							if ( !(strActualValue.equals("*")) && !(strActualValue.equals("")) ) {
								...
								if (strOperator.equals(OP_INCLUDES)) {
									if (strSelectFieldName.equals(RequirementsUtil.getContentTextAttribute(context)))
		...
		if("".equals(strActualContainedIn) || "null".equals(strActualContainedIn) || strActualContainedIn==null) {
			...
			while (st.hasMoreTokens() && (sQueryLimit == 0 || sQueryLimit > mapList.size())) {
				...
				MapList reqObjMapList = domObj.getRelatedObjects(context,RequirementsUtil.getSpecStructureRelationship(context),"*",objSelectList,null,false,true,(short)0,strObjWhereExp,null);
      ...
      if (strObjectId != null && strObjectId.length() > 0){
			...
			MapList excludeList = null;
				...
				sbRelSelect = sbRelSelect.append(RequirementsUtil.getSubRequirementRelationship(context))
                					 ...
                					 .append(RequirementsUtil.getDerivedRequirementRelationship(context));
				DomainObject dom = new DomainObject(strObjectId);
				excludeList = dom.getRelatedObjects(context, sbRelSelect.toString(), RequirementsUtil.getRequirementType(context),
...
*/
		...
		if ( strTitle!=null && (!strTitle.equals(SYMB_WILD)) && (!strTitle.equals("")) ) {
			...
			sbWhereExp.append(SYMB_ATTRIBUTE);
			sbWhereExp.append(SYMB_OPEN_BRACKET);
			sbWhereExp.append(RequirementsUtil.getTitleAttribute(context));
			sbWhereExp.append(SYMB_CLOSE_BRACKET);
			sbWhereExp.append(SYMB_MATCH);
			sbWhereExp.append(SYMB_QUOTE);
	...
	public static StringList getContextObjects(final Context context, final String[] args) throws Exception
	{
		...
		if((selectedProgram == null) ||("".equals(selectedProgram)))
		{
			...
			String relationshipPattern = PropertyUtil.getSchemaProperty(context, RequirementsConstants.SYMBOLIC_RELATIONSHIP_SPECIFICATION_STRUCTURE) + "," +
					PropertyUtil.getSchemaProperty(context, RequirementsConstants.SYMBOLIC_RELATIONSHIP_SUB_REQUIREMENT);
			String typePattern = RequirementsUtil.getRequirementType(context)+","+RequirementsUtil.getRequirementSpecificationType(context)+","+RequirementsUtil.getChapterType(context);
    ...
    public Map<String, String> updateValuesRMTTableType(Context context, String[] args) throws Exception {
        ...
        try {
            ...
            if (dmoParent.isKindOf(context, RequirementsUtil.getRequirementSpecificationType(context))) {
                valuesMap.put(RequirementsUtil.getRequirementType(context), RequirementsUtil.getRequirementType(context));
                valuesMap.put(RequirementsUtil.getChapterType(context), RequirementsUtil.getChapterType(context));
                valuesMap.put(RequirementsUtil.getCommentType(context), RequirementsUtil.getCommentType(context));
            }
            else if (RequirementsUtil.isRequirement(context, parentID)) {
                valuesMap.put(RequirementsUtil.getSubRequirementRelationship(context), RequirementsUtil.getSubRequirementRelationship(context));
                valuesMap.put(RequirementsUtil.getDerivedRequirementRelationship(context), RequirementsUtil.getDerivedRequirementRelationship(context));
            }
            else if (dmoParent.isKindOf(context, RequirementsUtil.getChapterType(context))) {
                valuesMap.put(RequirementsUtil.getRequirementType(context), RequirementsUtil.getRequirementType(context));
                valuesMap.put(RequirementsUtil.getChapterType(context), RequirementsUtil.getChapterType(context));
                valuesMap.put(RequirementsUtil.getCommentType(context), RequirementsUtil.getCommentType(context));
            }
            else if (dmoParent.isKindOf(context, RequirementsUtil.getCommentType(context))) {
    ...
    public String getAutoNameValueRMT(Context context, String[] args) {
        ...
        try {
            ...
            String admAlias = FrameworkUtil.getAliasForAdmin(context, "type", strType, true);
            strName = FrameworkUtil.autoName(context, admAlias, DomainConstants.EMPTY_STRING,
