...
public void floatReferenceDocumentsOnRelease(Context context, String[] args)
          throws Exception
  {
    ...
    //checking float to older revision key is disable or not 
      ...
      sNextRevConnectedImplementedCO="next.to["+RELATIONSHIP_IMPLEMENTED_ITEM+"].from.to["+ChangeConstants.RELATIONSHIP_CHANGE_ACTION+"|from.current != \""+completeState+"\"].from.id";
      sNextRevConnectedAffectedItemCO="next.to["+RELATIONSHIP_CHANGE_AFFECTED_ITEM+"].from.to["+ChangeConstants.RELATIONSHIP_CHANGE_ACTION+"|from.current != \""+completeState+"\"].from.id";
      ...
      while (partItr.hasNext())
      {
          ...
          if("True".equalsIgnoreCase(sFloatToOlderRevision)){
            ...
            sNextRevState = (String)partInfo.get("next.current");
  	    	...
  	    	sNextRevAssociatedCO = UIUtil.isNotNullAndNotEmpty((String)partInfo.get(sNextRevConnectedImplCO))?(String)partInfo.get(sNextRevConnectedImplCO):(String)partInfo.get(sNextRevConnectedAICO);
