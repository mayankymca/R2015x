   ...
   try {
    ...
    protected void migrateRequirementSpecificationRels(Context context,String[] args) throws Exception
    {
        try
        {
			...
			String strNewRelType = RequirementsUtil.getSpecStructureRelationship(context);
			...
			for(int i=0;i<resultsArray.length;i++)
			{
				...
				if (parsedResult.length == 7 )
				{
					...
					{
						migrationInfoWriter("Could not migrate Relationship of type " + RequirementsUtil.getRequirementSpecificationType(context) + " between " + fromName + " and " + toName + " because of invalid To Type.");
					}
				}				
			}
        }
	...
	protected void migrateOriginatingRequirementRels(Context context,String[] args) throws Exception
    {
        try
        {
            ...
            String strRelType = RequirementsUtil.getOriginatingRequirementRelationship(context);
			String strNewRelType = RequirementsUtil.getSpecStructureRelationship(context);
