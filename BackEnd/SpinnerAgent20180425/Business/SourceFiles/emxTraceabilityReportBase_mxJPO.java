...
/*
* @quickreview LX6     12:09:06 (IR-186487R2013x "NHI:V6R214:Function_026281: Requirement Parameter traceability report is KO. ")
* @quickreview JX5     13:04:10 Adapt code to use Type Icon Function & Type Icon Program
* @quickreview ZUD     13:06:18 IR-234701V6R2014x State column values in Full Traceability of Requirement are not translated into Japanese
* @quickreview T25 DJH 13:06:18 IR-234681V6R2014x.Added language support to "Sub Requirement" and "Derived Requiremnt".
* @quickreview T25 DJH 13:08:07 IR-207745V6R2014x. Modified method getExpressionColor(), added language support to "State" Column in excel during export.
* @quickreview T25 DJH 13:08:08 IR-207745V6R2014x. Integrating Qingwu's suggestion. Modified method getExpressionColor(), added language support to "Link Status" and "Validation Status" columns in excel during export.
* @quickreview JX5     14:09:16 IR-328029-3DEXPERIENCER2015x : STP: Requirement parameter traceability report KO.
* @quickreview JX5     14:10:03 Modify getParameterValue(Context context, String[] args) to retrieve the pameter value with its unit
...
import com.dassault_systemes.parameter_modeler.PlmParameter;
import com.dassault_systemes.parameter_modeler.PlmParameterUtil;
...
//End:V6R2010x:RMT Requirements Allocation Report
      ...
      throws Exception
   {
      ...
      if(mxType.isOfParentType(context, objectType, RequirementsUtil.getSpecificationType(context)))
      {
    	  objectType = RequirementsUtil.getSpecificationType(context);
      }
      if(mxType.isOfParentType(context, objectType, RequirementsUtil.getRequirementType(context)))
      {
    	  objectType = RequirementsUtil.getRequirementType(context);
      }
      if(mxType.isOfParentType(context, objectType, RequirementsUtil.getWorkspaceVaultType(context)))
      {
    	  objectType = RequirementsUtil.getWorkspaceVaultType(context);
      ...
      try
      {
         ...
         if (outList != null && outList.size() > 0)
         {
            ...
            if (showFolder)
            {
               ...
               folderMap.put(SELECT_RELATIONSHIP_ID, "0.0.0." + sectionId);
               folderMap.put(SELECT_TYPE, "");
               folderMap.put(SELECT_NAME, sectionName);
...
//               }
      ...
      throws Exception
   {
      ...
      String relationship = RequirementsUtil.getSpecStructureRelationship(context); // Default
      if(selectedObj!=null){
    	  relationship += ","+RequirementsUtil.getDerivedRequirementRelationship(context)+"" +
      ...
      if (reportTypes != null && reportTypes.length() > 0)
      {
    	  ...
    	  for (int ii = 0; ii < splitTypes.length; ii++)
    	  {
    		  ...
    		  StringList childTypes = RequirementsUtil.getDerivedTypesFromCache(reportType); //UINavigatorUtil.getChildrenTypeFromCache(reportType, false);
      ...
      final String SELECT_IS_KIND_OF_REQUIREMENT = "type.kindof[" + RequirementsUtil.getRequirementType(context) + "]";
      final String SELECT_IS_KIND_OF_CHAPTER = "type.kindof[" + RequirementsUtil.getChapterType(context) + "]";
      ...
      String expandTypes = RequirementsUtil.getRequirementType(context) + "," + RequirementsUtil.getChapterType(context);
      ...
      if (toker.countTokens() > 0)
      {
    	  ...
    	  while (toker.hasMoreTokens())
    	  {
    		  ...
    		  if (doSpec.isKindOf(context, RequirementsUtil.getRequirementType(context))) {
    	  ...
    	  else
    	  {
    		  ...
    		  if (tokerSource.countTokens() > 0)
    		  {
    			  ...
    			  while (tokerSource.hasMoreTokens())
    			  {
    				  ...
    				  boolean isRequirement = doBaseSpec.isKindOf(context, RequirementsUtil.getRequirementType(context));
      ...
      objSelects.addElement("attribute[" + RequirementsUtil.getStatusAttribute(context) + "]");
      objSelects.addElement("attribute[" + RequirementsUtil.getValidationStatusAttribute(context) + "]");
      objSelects.addElement("attribute[" + RequirementsUtil.getNotesAttribute(context) + "]");
      objSelects.addElement("attribute[Marketing Name]");
      objSelects.addElement("attribute[" + RequirementsUtil.getTitleAttribute(context) + "]");  // Added in R210
      ...
      relSelects.addElement("attribute[" + RequirementsUtil.getLinkStatusAttrubite(context) + "]");
      ...
      for (int jj = 0; jj < vReqList1.size(); jj++)
      {
    	  ...
    	  short expand = 0;
    	  if(extReportRels.contains(RequirementsUtil.getCloneRelationship(context))||selectedObj!=null||extReportRels.contains(RequirementsUtil.getDerivedRequirementRelationship(context))){
    		  expand = 1;
    	  }
   ...
   private void getlinkStatutusValuesFromArrayList(Context context, String expValue, ArrayList<String> idList, Vector linkStatusValues) throws MatrixException{
   ...
   private List getLinkStatuAttribute(Context context, MapList rowData) throws Exception{
	   return getLinkStatuAttribute(context, rowData, "attribute[Link Status]");
   ...
   private List getLinkStatuAttribute(Context context, MapList rowData, String expValue) throws Exception{
	   ...
	   for(int i=0; i<rowDatasize;i++){
		   ...
		   if(id != null){
			   ...
			   //by an empty string
			   getlinkStatutusValuesFromArrayList(context, expValue, idList, linkStatusValues);
   ...
   {
       return getTableColumnData(context, dataList, attrName, false);
   }
    ...
    private List getTableColumnData(Context context, MapList dataList, String attrName, boolean highlightMissingValue)
    {
   ...
   */
	   ...
	   throws Exception
   {
	   ...
	   List attList = getTableColumnData(context, theData, attName, highlightMissingValue);
	...
	adminType = "attribute_ValidationStatus";
	   ...
	   if (renderHTML && reportHTML && !forceNoHref)
	   {
		   ...
		   for (int ii = 0; ii < iconOIDs.size(); ii++)
		   {
			   ...
			   StringList lstChildren = ProductLineUtil.getChildrenTypes(context, RequirementsUtil.getRequirementType(context));
			   lstChildren.add(RequirementsUtil.getRequirementType(context));
...
//            objHtml += "<td>";
            try{
	            ...
	            objHtml += "<b>" + objName + (objRev == null? "": XSSUtil.encodeForHTML(context, objRev)) + "</b>";
	            if (hyperLink && objOID != null && objOID.length() > 0)
	               objHtml += "</a>";
	//            objHtml += "</td>";
	//            objHtml += "</tr>";
	//            objHtml += "</table>";
      ...
      String strHTMLObjectName = "<div style='display:none'>" + objName + "</div>";
      ...
      throws Exception
   {
      ...
      return(expandParentLinkList(context, reqList, null, columnMap, RequirementsUtil.getSpecStructureRelationship(context), "*Specification*", null, renderHTML, hyperLink));
   }
      ...
      throws Exception
   {
      ...
      String relTypes = RequirementsConstants.RELATIONSHIP_PARAMETER_AGGREGATION + ","+RequirementsConstants.RELATIONSHIP_PARAMETER_USAGE;
      ...
      throws Exception
   {
      ...
      List specList = expandParentLinkList(context, reqList, null, null, RequirementsUtil.getSpecStructureRelationship(context), "*Specification*", TRACEABILITY_NAME_ONLY, false, false, isMatrixSearch);
   ...
   throws Exception
   {
       ...
       expandSelects.addElement(SELECT_ID);
       expandSelects.addElement(SELECT_TYPE);
       String expandTypes = RequirementsUtil.getRequirementType(context);
       ...
       resultsList = DomainObject.findObjects(context, RequirementsUtil.getRequirementType(context), vaultString, whereString, resultSelects);
       ...
       if(specStructId != null)
       {
        ...
        if (strObjectId == null) {
            reqList = DomainObject.findObjects(context, RequirementsUtil.getProductsType(context),
	   ...
	   // Handled separate For loop for each context. For Bug 378038, 378045
            ...
            if(prodObj.isKindOf(context,RequirementsUtil.getFeatureType(context)))
            {
            ...
            else if(prodObj.isKindOf(context,RequirementsUtil.getRequirementSpecificationType(context)) || prodObj.isKindOf(context,RequirementsUtil.getChapterType(context)))
            {
            ...
            // In the context of Model
                ...
                String strRelName = RequirementsUtil.getCandidateItemRelationship(context);
                String strTypeName = RequirementsUtil.getFeaturesType(context);
                StringList objectSelects = new StringList(DomainConstants.SELECT_ID);
                StringList relSelects = new StringList(DomainConstants.SELECT_RELATIONSHIP_ID);
                short sLevel = 0;
            ...
            {
                reqList = prodObj.getRelatedObjects(context, RequirementsUtil.getProductRequirementRelationship(context),
   ...
   throws Exception
   {
	   ...
	   StringList slSourceSpecIds = new StringList();
           ...
           while (tokenizer.hasMoreTokens())
           {
               ...
               boolean isSpec = dmoSource.isKindOf(context, RequirementsUtil.getRequirementSpecificationType(context));
               if (isSpec)
               {
	   ...
	   tokenizer = new StringTokenizer(targertSpecObjs, ",", false);
           while (tokenizer.hasMoreTokens())
           {
               ...
               boolean isSpec = dmoTarget.isKindOf(context, RequirementsUtil.getRequirementSpecificationType(context));
               if (isSpec)
               {
   ...
   private MapList updateWithSpecInfo (Context context, MapList tableData, String strObjectIdSelect, StringList slSpecSelect, StringList slValidSpecIds, String strResultMapKeyName) throws Exception
   {
       try
       {
	   final String SELECT_IS_KIND_OF_SPECIFICATION = "type.kindof[" + RequirementsUtil.getSpecificationType(context) + "]";
	   ...
	   sbTypePattern.append(RequirementsUtil.getRequirementType(context))
	   		.append(",")
	   		.append(RequirementsUtil.getChapterType(context))
	   		.append(",")
	   		.append(RequirementsUtil.getCommentType(context))
	   		.append(",")
	   		.append(RequirementsUtil.getSpecificationType(context));
	   StringBuffer sbRelPattern = new StringBuffer(128);
	   sbRelPattern.append(RequirementsUtil.getSpecStructureRelationship(context))
	   		.append(",")
       ...
       try
       {
           for (int i=0; i < mlObjectList.size(); i++)
           {
		       ...
		       //
			       ...
			       {
				   // Find the icon
					   String strTypeIcon = emxRMTCommon_mxJPO.getRelIconProperty(context, RequirementsUtil.getDerivedRequirementRelationship(context));
					   ...
					   for (Iterator itrParentSpecInfo = mlParentSpecInfo.iterator(); itrParentSpecInfo.hasNext();)
					   {
						   ...
						   if (isExportFormat && !isPrinterFriendly)
						   {
							   ...
							   else
							   {
								   ...
								   strHTML = FrameworkUtil.findAndReplace(strHTML, "${NAME}", XSSUtil.encodeForHTML(context, strSpecName));
							   }
						   }
						   else
						   {
   ...
   public List getRequirementSpecificationTargetName(Context context, String[] args) throws Exception
   {
       ...
       StringBuffer sbSpecChildTypes= new StringBuffer(RequirementsUtil.getSpecificationType(context));
          ...
          if(strObjId !=null)
          {
               ...
               if( strTargetId != null)
               {
                   ...
                   specObjMapList   = domObj.getRelatedObjects(context,
                                           RequirementsUtil.getSpecStructureRelationship(context),    // relPattern
   ...
   public Vector getDerivedRequirementLinkStatusColumnData(Context context, String[] args) throws Exception
   {
       ...
       AttributeType attributeType = new AttributeType(RequirementsUtil.getLinkStatusAttrubite(context));
       attributeType.open(context);
       StringList slLinkStatusRanges = attributeType.getChoices(context);
       attributeType.close(context);
       StringList slTranslatedLinkStatusRanges = com.matrixone.apps.domain.util.i18nNow.getAttrRangeI18NStringList(RequirementsUtil.getLinkStatusAttrubite(context),
	       slLinkStatusRanges,
               strLanguage);

       MapList mlObjectList = (MapList)programMap.get("objectList");
       Vector vecColumnData = new Vector(mlObjectList.size());
       final String SELECT_ATTRIBUTE_LINK_STATUS = "attribute[" + RequirementsUtil.getLinkStatusAttrubite(context) + "]";
       try{
           for (int i=0; i < mlObjectList.size(); i++) {
               Map mapRow = (Map)mlObjectList.get(i);
               Map mapCol = (Map)mapRow.get(nColumnIndex);
...
throws Exception
 {
	...
	String sExpandTypes = RequirementsUtil.getRequirementType(context) + "," + RequirementsUtil.getChapterType(context);
	...
	if (targetChapterRequirementObjs == null
		...
		while (stTargettoken.hasMoreTokens()) {
		    ...
		    mapTargetList = doTargetSpecIds.getRelatedObjects(context,RequirementsUtil.getSpecStructureRelationship(context), sExpandTypes, slExpandSelects, null,
			...
			String strTargetObjType = (String) mapTargetIds
				.get(SELECT_TYPE);
			if (!strTargetObjType.equals(RequirementsUtil.getChapterType(context))
		...
		if(strChapOrRequirement.equals(RequirementsUtil.getRequirementType(context)))
		{
		    mlTargetMapNew.add(strChapOrReqId);
		}
		else if(strChapOrRequirement.equals(RequirementsUtil.getChapterType(context)))
		{
		    mapTargetList = doChapOrReqIds.getRelatedObjects(context,RequirementsUtil.getSpecStructureRelationship(context), sExpandTypes, slExpandSelects, null,
			    false, true, (short) 0, null, null);
	...
	int allSize = 0;
	strRelWhere = "attribute[" + RequirementsUtil.getLinkStatusAttrubite(context) + "]!='Invalid'"; //TODO remove Hardcoding
	...
	while (stSourceToken.hasMoreTokens()) {
	    ...
	    mapSourceList = doSourceReqIds.getRelatedObjects(context,RequirementsUtil.getSpecStructureRelationship(context), sExpandTypes,
		...
		if (!strsourceObjType.equals(RequirementsUtil.getChapterType(context))
		...
		//IR-160176V6R2013x
		if(reportTo){
			mapSourceDerivedRequirement = doRequirementIds.getRelatedObjects(context,sExtNameReportRels, RequirementsUtil.getRequirementType(context), slExpandSelects, null,
			reportTo, false, (short) 0, null, strRelWhere);
		}
		if(reportFrom){
			MapList mapFrom = doRequirementIds.getRelatedObjects(context,sExtNameReportRels, RequirementsUtil.getRequirementType(context), slExpandSelects, null,
 ...
 */
	public Object getTargetTitle(Context context, String args[]) throws Exception
	{
	   ...
	   {
		   String expandRel = RequirementsUtil.getSpecStructureRelationship(context) + ",";
		   expandRel += RequirementsCommon.getBooleanPreference(context, RequirementsCommon.PREF_REPORT_REQ_TESTCASE_INC_DERIVED_REQ) ? RequirementsUtil.getDerivedRequirementRelationship(context) : "";
		   expandRel += expandRel.length() > 0 ? "," : "";
		   expandRel += RequirementsCommon.getBooleanPreference(context, RequirementsCommon.PREF_REPORT_REQ_TESTCASE_INC_SUB_REQ) ? RequirementsUtil.getSubRequirementRelationship(context) : "";
		   return expandRel;
	   }
	   ...
	   {
		   String expandRel = RequirementsUtil.getSpecStructureRelationship(context) + ",";
		   expandRel += RequirementsCommon.getBooleanPreference(context, RequirementsCommon.PREF_REPORT_REQ_FEATURE_INC_DERIVED_REQ) ? RequirementsUtil.getDerivedRequirementRelationship(context) : "";
		   expandRel += expandRel.length() > 0 ? "," : "";
		   expandRel += RequirementsCommon.getBooleanPreference(context, RequirementsCommon.PREF_REPORT_REQ_FEATURE_INC_SUB_REQ) ? RequirementsUtil.getSubRequirementRelationship(context) : "";
		   return expandRel;
	   }
	   ...
	   public  static Object getParameterValue(Context context, String[] args)
				 throws Exception
				 {
					 Vector returnList = new Vector();
					   ...
					   PlmParameterUtil paramUtil = new PlmParameterUtil();
					   if(objectListMap!=null)
					   {
						   ...
						   for (int i = 0; i < size; i++)
						   {					
							   PlmParameter param = new PlmParameter(context, busID[i]);
							   StringList value = paramUtil.getParameterValuesWithUnit(context, busID[i], PlmParameterUtil.VALUE_ATTRIBUTE); 

							   if(value != null) {
								   String unit = (String)value.get(1);

								   if(unit.length() != 0)
									   returnList.add(value.get(0) + " " + paramUtil.getUnitNLS(context, unit));
								   else{

									   if(param.getParameterType(context).equals("Boolean")){
										   returnList.add(paramUtil.getBooleanNLS(context, (String) value.get(0)));
									   }
									   else
										   returnList.add(value.get(0));
								   }
							   }
							   else
								   returnList.add(null);
...
//End:LX6:display Clone information feature
	...
	public List getStatusLinkInformation(Context context, String[] args) throws Exception{
		...
		for(int i=0;i<theData.size();i++){
			...
			linkStatus[i] = DomainRelationship.newInstance(context, relId).getAttributeValue(context, RequirementsUtil.getLinkStatusAttrubite(context));
