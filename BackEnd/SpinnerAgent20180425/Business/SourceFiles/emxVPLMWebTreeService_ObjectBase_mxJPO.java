...
public class emxVPLMWebTreeService_ObjectBase_mxJPO extends emxDomainObject_mxJPO {
	...
	private MapList putVPLMObjectsOnJSONObject(Context context, JSONObject object) throws MatrixException{
		...
		if(strTypeName.equalsIgnoreCase(RequirementsUtil.getRequirementSpecificationType(context))||strTypeName.equalsIgnoreCase(RequirementsUtil.getRequirementType(context))){
	...
	private String getChildrenObjectTypesFromId(Context context, String objectId) throws FrameworkException{
		...
		if(RequirementsUtil.getRequirementSpecificationType(context).equalsIgnoreCase(strObjectType)||
			RequirementsUtil.getChapterType(context).equalsIgnoreCase(strObjectType)){
			strObjTypes = RequirementsUtil.getRequirementType(context) + "," +
            RequirementsUtil.getChapterType(context) + "," +
            RequirementsUtil.getCommentType(context);
		}else if(RequirementsUtil.getRequirementType(context).equalsIgnoreCase(strObjectType)){
			strObjTypes = RequirementsUtil.getRequirementType(context);
		}else if(RequirementsUtil.getCommentType(context).equalsIgnoreCase(strObjectType)){
			strObjTypes = "";
		}else{
			strObjTypes = null;
		}
	...
	private String getToRelTypesFromId(Context context, String objectId, int depth) throws FrameworkException{
		...
		if(RequirementsUtil.getRequirementSpecificationType(context).equalsIgnoreCase(strObjectType)||
			RequirementsUtil.getChapterType(context).equalsIgnoreCase(strObjectType)){
			if(depth == 1){
				strRelTypes = 	RequirementsUtil.getSpecStructureRelationship(context);
			}else{
				strRelTypes = 	RequirementsUtil.getSpecStructureRelationship(context) + "," + 
			...
			strRelTypes = RequirementsUtil.getSubRequirementRelationship(context)+ "," +
							RequirementsUtil.getDerivedRequirementRelationship(context);;
		}else if(RequirementsUtil.getCommentType(context).equalsIgnoreCase(strObjectType)){
			strRelTypes = "";
		}else{
	...
	private String getParentObjectTypesFromId(Context context, String objectId) throws FrameworkException{
		...
		if(RequirementsUtil.getRequirementSpecificationType(context).equalsIgnoreCase(strObjectType)){
			strObjTypes = "";
		}else if(RequirementsUtil.getChapterType(context).equalsIgnoreCase(strObjectType)){
			strObjTypes =   RequirementsUtil.getChapterType(context) + "," +
							RequirementsUtil.getRequirementSpecificationType(context);
		}else if(RequirementsUtil.getRequirementType(context).equalsIgnoreCase(strObjectType)){
			strObjTypes =   RequirementsUtil.getRequirementType(context)+ "," +
							RequirementsUtil.getChapterType(context) + "," +
							RequirementsUtil.getRequirementSpecificationType(context);
	...
	private JSONObject getTestCaseUseCaseObjects(Context context, String strObjectId) throws Exception{
        ...
        String strRelTypes = RequirementsUtil.getRequirementValidationRelationship(context)+ "," +
		  					 RequirementsUtil.getSubTestCaseRelationship(context)/* + "," +  
		  					 RequirementsUtil.getSubUseCaseRelationship(context) + "," +
        					 RequirementsUtil.getRequirementUseCaseRelationship(context)*/;
        					  
        
        String strObjTypes = RequirementsUtil.getTestCaseType(context)/* + "," +
        					 RequirementsUtil.getUseCaseType(context)*/;
