
tcl;

eval {
   if {[info host] == "mostermant61p" } {
      source "c:/Program Files/TclPro1.3/win32-ix86/bin/prodebug.tcl"
   	  set cmd "debugger_eval"
   	  set xxx [debugger_init]
   } else {
   	  set cmd "eval"
   }
}
$cmd {

#  Set up array for symbolic name mapping
#
   set lsPropertyName [mql get env PROPERTYNAME]
   set lsPropertyTo [mql get env PROPERTYTO]
   set sTypeReplace "command "

   foreach sPropertyName $lsPropertyName sPropertyTo $lsPropertyTo {
      set sSchemaTest [lindex [split $sPropertyName "_"] 0]
      if {$sSchemaTest == "command"} {
         regsub $sTypeReplace $sPropertyTo "" sPropertyTo
         regsub "_" $sPropertyName "|" sSymbolicName
         set sSymbolicName [lindex [split $sSymbolicName |] 1]
         array set aSymbolic [list $sPropertyTo $sSymbolicName]
      }
   }

   set sFilter [mql get env 1]
   set bTemplate [mql get env 2]
   set bSpinnerAgentFilter [mql get env 3]
   set sGreaterThanEqualDate [mql get env 4]
   set sLessThanEqualDate [mql get env 5]

   set sAppend ""
   if {$sFilter != ""} {
      regsub -all "\134\052" $sFilter "ALL" sAppend
      regsub -all "\134\174" $sAppend "-" sAppend
      regsub -all "/" $sAppend "-" sAppend
      regsub -all ":" $sAppend "-" sAppend
      regsub -all "<" $sAppend "-" sAppend
      regsub -all ">" $sAppend "-" sAppend
      regsub -all " " $sAppend "" sAppend
      set sAppend "_$sAppend"
   }
   
   if {$sGreaterThanEqualDate != ""} {
      set sModDateMin [clock scan $sGreaterThanEqualDate]
   } else {
      set sModDateMin ""
   }
   if {$sLessThanEqualDate != ""} {
      set sModDateMax [clock scan $sLessThanEqualDate]
   } else {
      set sModDateMax ""
   }
   
   set sSpinnerPath [mql get env SPINNERPATH]
   if {$sSpinnerPath == ""} {
      set sOS [string tolower $tcl_platform(os)];
      set sSuffix [clock format [clock seconds] -format "%Y%m%d"]
      
      if { [string tolower [string range $sOS 0 5]] == "window" } {
         set sSpinnerPath "c:/temp/SpinnerAgent$sSuffix";
      } else {
         set sSpinnerPath "/tmp/SpinnerAgent$sSuffix";
      }
      file mkdir $sSpinnerPath
   }

   set sPath "$sSpinnerPath/Business/SpinnerCommandData$sAppend.xls"
   #KYB Start
   set sSourceFileDir "$sSpinnerPath/Business/SourceFiles"
   #KYB End
   set lsCommand [split [mql list command $sFilter] \n]
   set sFile "Name\tRegistry Name\tDescription\tLabel\tHref\tAlt\tSetting Name (use \"|\" delim)\tSetting Value (use \"|\" delim)\tUsers (use \"|\" delim)\tHidden (boolean)\tCode\tIcon File\n"
   set sMxVersion [mql get env MXVERSION]
   if {$sMxVersion == ""} {
      set sMxVersion "2012"
   }
   
   if {!$bTemplate} {
      foreach sCommand $lsCommand {
         set bPass TRUE
         set sModDate [mql print command $sCommand select modified dump]
         set sModDate [clock scan [clock format [clock scan $sModDate] -format "%m/%d/%Y"]]
         if {$sModDateMin != "" && $sModDate < $sModDateMin} {
            set bPass FALSE
         } elseif {$sModDateMax != "" && $sModDate > $sModDateMax} {
            set bPass FALSE
         }
         
         if {($bPass == "TRUE") && ($bSpinnerAgentFilter != "TRUE" || [mql print command $sCommand select property\[SpinnerAgent\] dump] != "")} {
            set sName [mql print command $sCommand select name dump]
            set sOrigName ""
            catch {set sOrigName $aSymbolic($sCommand)} sMsg
            regsub -all " " $sCommand "" sOrigNameTest
            if {$sOrigNameTest == $sOrigName} {
               set sOrigName $sCommand
            }
            set sDescription [mql print command $sCommand select description dump]
            set sLabel [mql print command $sCommand select label dump]
            set sHref [mql print command $sCommand select href dump]
            set sAlt [mql print command $sCommand select alt dump]
            set sHidden [mql print command $sCommand select hidden dump]
            set sCode [mql print command $sCommand select code dump]
            regsub -all "\134\012" $sCode "<NEWLINE>" sCode
            regsub -all "\134\011" $sCode "<TAB>" sCode            
            regsub -all "\134\042" $sCode "<DQUOTE>" sCode            
            set slsSettingName [mql print command $sCommand select setting.name dump " | "]
            set slsSettingValue [mql print command $sCommand select setting.value dump " | "]
			
			#KYB - Traverse thr each setting of a command, look for char '|' 
		    set slsSettingValue ""
		    set iCnt2 0
		    set sListCommandSettingName [split $slsSettingName |]  
		    set iCnt1 [ llength $sListCommandSettingName ]
		    foreach sCommandSettingName $sListCommandSettingName {
				 set sCommandSettingName [string trim $sCommandSettingName]
				 set slsCommandSettingValue [ mql print command $sCommand select setting\[$sCommandSettingName\].value dump ]
				 regsub -all "\134|" $slsCommandSettingValue "<PIPE:>" slsCommandSettingValue
				 regsub -all "<PIPE:><PIPE:>" $slsCommandSettingValue "||" slsCommandSettingValue				 
				 append slsSettingValue $slsCommandSettingValue
				 incr iCnt2
				 if { $iCnt2 != $iCnt1 } { append slsSettingValue " | " }
		    }
			
            set slsUser [mql print command $sCommand select user dump " | "]
			#KYB Start
			set bCommandFile [mql get env COMMMANDCODE]
			if { $bCommandFile == "TRUE" } {
				set sCmdActualCode [mql print command $sCommand select code dump]
				set sCmdActualCode [string trim $sCmdActualCode]
				if { $sCmdActualCode != "" } {
					set cmdCodeFile $sCommand
					append cmdCodeFile ".json"
					mql print command $sCommand select code dump output "$sSourceFileDir/$cmdCodeFile"
				}
			}
			#KYB End
            append sFile "$sName\t$sOrigName\t$sDescription\t$sLabel\t$sHref\t$sAlt\t$slsSettingName\t$slsSettingValue\t$slsUser\t$sHidden\t$sCode\n"
         }
      }
   }
   set iFile [open $sPath w]
   puts $iFile $sFile
   close $iFile
   puts "Command data loaded in file $sPath"
}
